...
class TestRunner:
    ...
    def run_all_tests(self, selected_types: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        if self.config["parallel"]:
            ...
            with ThreadPoolExecutor() as executor:
                futures = {executor.submit(self.run_test, test_type): test_type for test_type in types_to_run}
    ...
    def extract_detailed_test_results(self, test_type: str, report_data: Any) -> List[Dict[str, Any]]:
            ...
            if test_type == "unit" or test_type == "integration" or test_type == "e2e":
                ...
                if "tests" in report_data:
                    for test_data in report_data.get("tests", []):
                        test_result = {
                            ...
                            "error": test_data.get("call", {}).get("crash", {}).get("message", ""),
                            "traceback": test_data.get("call", {}).get("crash", {}).get("traceback", ""),
                            "output": test_data.get("stdout", "") + test_data.get("stderr", "")
                        }
                ...
                for feature in report_data:
                    ...
                    for element in feature.get("elements", []):
                        scenario_name = f"{feature_name}: {element.get('name', 'Unknown Scenario')}"
                        failed_steps = []
                        duration = 0
                        output = ""
                        ...
                        for step in element.get("steps", []):
                            ...
                            duration += step_result.get("duration", 0) / 1e9  # Convert nanoseconds to seconds
                        ...
                        test_result = {
                            ...
                            "duration": duration,
                            "error": "\n".join([f"Step: {s['step']}\nError: {s['error']}" for s in failed_steps]),
                            "output": output
                        }
                        tests.append(test_result)
                ...
                for suite in report_data.get("suites", []):
                    for spec in suite.get("specs", []):
                        test_result = {
                            ...
                            "duration": spec.get("duration", 0) / 1000,  # Convert ms to seconds
                            "error": "\n".join([error.get("message", "") for error in spec.get("errors", [])]),
                            "output": "\n".join(spec.get("stdout", []) + spec.get("stderr", []))
                        }
                        tests.append(test_result)
        except Exception as e:
    ...
    def generate_detailed_report(self, test_type: str, result: Dict[str, Any]) -> str:
        ...
        if template_path.exists():
            ...
            print(f"Detailed report template not found at {template_path}, using default template")
            # Default simple template would be defined here
            template_content = "<html><body><h1>{{ test_type_name }} Results</h1></body></html>"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Run IoTSphere tests and generate reports")
    parser.add_argument("--types", nargs="+", choices=TEST_TYPES.keys(),
