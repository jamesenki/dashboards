...
def main():
    ...
    try:
        ...
        for model_version in model_versions:
            ...
            if metrics_count > 0:
                logger.info(f"Metrics already exist for {model_name} ({model_id}) version {version} - {metrics_count} records found")
                continue
            
            logger.info(f"Adding sample metrics for {model_name} ({model_id}) version {version}")
            ...
            logger.info(f"Successfully added {len(metrics_to_insert)} metrics for {model_name} ({model_id}) version {version}")
