...
class VendingMachineOperationsService:
    ...
    def get_sales_data(self, machine_id: str, period: Optional[SalesPeriod] = None) -> SalesData:
    ...
    def _get_alerts(self, machine: VendingMachine, 
                   sales_data: SalesData,
                   temperature_trends: TemperatureTrends) -> List[AlertModel]:
        ...
        if (temperature_trends and 
            temperature_trends.current_temperature is not None and 
            temperature_trends.current_temperature > 5):
            alerts.append(AlertModel(
                alert_id=str(uuid.uuid4()),
        ...
        # Check sales data
        if (sales_data and 
            sales_data.total_sales is not None and 
    ...
    def _generate_mock_sales_data(self, machine: VendingMachine, 
                                 period: Optional[SalesPeriod] = None) -> SalesData:
        ...
        total_revenue = total_sales * avg_price * (0.9 + 0.2 * random.random())  # Add some variation
        ...
        if machine.products:
            ...
            for product in machine.products:
                sales_count = int(total_sales * random.random() * 0.5)  # Random portion of total sales
                product_sales.append(ProductSale(
            ...
            product_names = ["Ice Cream - Vanilla", "Ice Cream - Chocolate", "Ice Cream - Strawberry", 
                            "Ice Cream - Mint", "Ice Cream - Cookie Dough"]
            ...
            for i, name in enumerate(product_names):
                ...
                product_price = 2.0 + random.random() * 1.5  # Random price between $2.00 and $3.50
                product_sales.append(ProductSale(
                    product_id=f"P{i+1}",
                    product_name=name,
                    sales_count=sales_count,
    ...
    def _generate_mock_maintenance_history(self, machine: VendingMachine) -> MaintenanceHistory:
        ...
        return MaintenanceHistory(
            ...
            next_scheduled_maintenance=datetime.now() + timedelta(days=random.randint(30, 90))
        )
    ...
    def _generate_mock_refill_history(self, machine: VendingMachine) -> RefillHistory:
        ...
        for i in range(num_events):
            ...
            events.append(RefillEvent(
                ...
                items=items,
                notes="Regular refill" if random.random() > 0.2 else "Emergency refill - low stock",
                timestamp=event_date
            ))
    ...
    def _generate_mock_temperature_trends(self, machine: VendingMachine) -> TemperatureTrends:
        ...
        base_temp = -18 + random.randint(0, 4)  # Base temperature between -18°C and -14°C
        ...
        avg_temperature = round(mean(r.temperature for r in readings), 1) if readings else base_temp
        
        # Calculate min and max temperatures
        min_temperature = round(min(r.temperature for r in readings), 1) if readings else base_temp - 2
        max_temperature = round(max(r.temperature for r in readings), 1) if readings else base_temp + 2
