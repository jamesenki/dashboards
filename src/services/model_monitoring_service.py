...
class ModelMonitoringService:
    ...
    def get_models(self, include_archived=False):
        ...
        for model in models:
            ...
            if (
                not hasattr(model, "accuracy")
                or model.accuracy is None
                or model.accuracy == "N/A"
            ):
                model.accuracy = 0.0

            if (
                not hasattr(model, "drift_score")
                or model.drift_score is None
                or model.drift_score == "N/A"
            ):
                model.drift_score = 0.0

            # Set health status based on metrics
            if hasattr(model, "health") and (
                model.health is None or model.health == "Unknown"
            ):
                if model.accuracy > 0 or model.drift_score > 0:
                    model.health = (
                        "Good"
                        if model.accuracy > 0.7 and model.drift_score < 0.3
                        else "Poor"
                    )
                else:
