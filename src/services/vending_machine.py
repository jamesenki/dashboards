...
class VendingMachineService:
    ...
    def create_vending_machine(
        ...
        vending_machine = VendingMachine(
            ...
            else datetime.fromisoformat(last_maintenance_date),
            next_maintenance_date=None
            if next_maintenance_date is None
            else datetime.fromisoformat(next_maintenance_date),
            products=[],
        )
    ...
    def get_vending_machine(self, vm_id: str) -> VendingMachine:
        ...
        if self.db:
            try:
                ...
                logging.warning(
                    f"Error retrieving vending machine {vm_id} from database: {e}"
                )
        ...
        if not vending_machine:
            ...
            sub_locations = {
                LocationType.RETAIL: [
                    SubLocation.HALLWAY,
                    SubLocation.LOBBY,
                    SubLocation.DINING_AREA,
                    SubLocation.WAITING_AREA,
                ],
                LocationType.OFFICE: [
                    SubLocation.LOBBY,
                    SubLocation.BREAK_ROOM,
            ...
            # Use hash for consistent, deterministic selection rather than trying to convert the ID to an integer
            maintenance_partner = maintenance_partners[
                hash(vm_id) % len(maintenance_partners)
            ]
    ...
    def get_all_vending_machines(self) -> List[VendingMachine]:
        ...
        logging.error(
            f"Database returned {len(machines)} machines: {[m.id for m in machines]}"
        )
        return machines
    ...
    def update_product_quantity(
        self, vm_id: str, product_id: str, quantity_change: int
    ) -> VendingMachine:
