...
class WaterHeaterOperationsService:
    ...
    async def get_operations_dashboard(self, heater_id: str) -> Dict[str, Any]:
        ...
        # Process gauge data
        dashboard_data["gauges"][
            "temperature"
        ] = self._transform_temperature_gauge_data(
        ...
        if latest_reading:
            ...
            dashboard_data["gauges"][
                "energy_usage"
            ] = self._transform_energy_usage_gauge_data(
                latest_reading.energy_usage
                if latest_reading.energy_usage is not None
                else 0
            )

            dashboard_data["gauges"][
                "flow_rate"
            ] = self._transform_flow_rate_gauge_data(
                latest_reading.flow_rate
                if latest_reading.flow_rate is not None
                else 0.0
            )
        ...
        dashboard_data["asset_health"] = self._calculate_asset_health(
            ...
            if latest_reading and latest_reading.pressure is not None
            else 0.0,
            latest_reading.energy_usage
            if latest_reading and latest_reading.energy_usage is not None
            else 0,
            latest_reading.flow_rate
    ...
    def _transform_temperature_gauge_data(
        ...
        if max_temp == min_temp:  # Avoid division by zero
            ...
            percentage = round(
                ((current_temp - min_temp) / (max_temp - min_temp)) * 100, 1
            )
    ...
    def _transform_energy_usage_gauge_data(
    ...
    def _transform_flow_rate_gauge_data(
        self, current_flow_rate: float
    ) -> Dict[str, Any]:
        """
