...
class PredictionService:
    ...
    async def get_prediction(
        self, device_id: str, prediction_type: str, force_refresh: bool = False
        ...
        try:
            ...
            default_prediction = self._create_default_prediction(
                device_id, prediction_type
            )
            ...
            logger.error(
                f"Failed to generate prediction for {prediction_type}: {str(e)}"
            )
            ...
            if os.getenv("ENVIRONMENT", "development") == "development":
                logger.info(
                    f"Returning default prediction for {prediction_type} in development mode"
                )
                return self._create_default_prediction(device_id, prediction_type)
            return None

    def _create_default_prediction(
        self, device_id: str, prediction_type: str
    ) -> PredictionResult:
        """
    ...
    async def _get_advanced_prediction_data(
        self, device_id: str, prediction_type: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        if prediction_type == "anomaly_detection":
                ...
                [
                    {
                        ...
                        if hasattr(reading, "pressure") and reading.pressure is not None
                        else 0.0,
                        "energy_usage": reading.energy_usage
                        if hasattr(reading, "energy_usage")
                        and reading.energy_usage is not None
                        else 0.0,
                        "flow_rate": reading.flow_rate
                        if hasattr(reading, "flow_rate")
            ...
            base_data["installation_location"] = getattr(
                water_heater, "location", "Unknown"
            )
            ...
            base_data["heater_type"] = getattr(
                water_heater, "heater_type", "Residential"
            )
            base_data["status"] = getattr(water_heater, "status", "ONLINE")
            base_data["efficiency_rating"] = getattr(
                water_heater, "efficiency_rating", 0.9
            )

        return base_data

    async def _get_water_heater_lifespan_data(
        self, device_id: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            if hasattr(self.water_heater_service, "get_history"):
                ...
                logger.info(
                    f"get_history method not available, using default history for {device_id}"
                )
                history = []
        except Exception as e:
        ...
        features = {
            ...
            "model": getattr(
                water_heater, "model_name", water_heater.name
            ),  # Use name as fallback
            "temperature_settings": [
                water_heater.target_temperature
            ],  # Store as a list to prevent iteration errors
            "total_operation_hours": getattr(
                water_heater, "total_operation_hours", 8760
            ),  # Default to 1 year if not available
            "water_hardness": getattr(
                water_heater, "water_hardness", 7.0
            ),  # Default to medium hardness if not available
        ...
        if (
            hasattr(water_heater, "maintenance_history")
            and water_heater.maintenance_history
        ):
        ...
        if history:
            ...
            if len(history) >= 7:  # At least a week of data
                ...
                if "temperature_readings" in history[0]:
                    temp_variations = [
                        max(day["temperature_readings"])
                        - min(day["temperature_readings"])
                        for day in history
                        if "temperature_readings" in day and day["temperature_readings"]
                    ]
                    if temp_variations:
