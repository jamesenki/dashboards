...
class VendingMachineOperationsServiceDB:
    ...
    async def get_vm_operations(self, vm_id: str) -> Dict[str, Any]:
        ...
        if hasattr(vm, 'readings') and vm.readings:
            ...
            sorted_readings = sorted(vm.readings, key=lambda r: r.timestamp, reverse=True)
        ...
        # Cache the result
        if self.ops_cache:
            await self.ops_cache.cache_vending_machine_operations(vm_id, operations_data)
    ...
    async def get_ice_cream_operations(self, machine_id: str) -> Dict[str, Any]:
        ...
        # Try to get from cache first
        if self.ops_cache:
            cached_data = await self.ops_cache.get_ice_cream_machine_operations(machine_id)
            if cached_data:
        ...
        for product in vm.products:
            ...
            if "ice cream" in product.name.lower() or product.category.lower() in ["dessert", "frozen", "ice cream"]:
                flavor_name = product.name
            else:
                # Convert regular product to ice cream flavor
                flavor_words = ["Vanilla", "Chocolate", "Strawberry", "Mint", "Cookies & Cream", 
                               "Caramel", "Butterscotch", "Blueberry", "Mango", "Coffee"]
                descriptor_words = ["Creamy", "Deluxe", "Premium", "Rich", "Smooth", "Classic", "Swirl"]
        ...
        door_closed = True
        if hasattr(vm, 'readings') and vm.readings:
            latest_reading = sorted(vm.readings, key=lambda r: r.timestamp, reverse=True)[0]
            door_closed = latest_reading.door_status == "CLOSED"
        
        # Build the operations dashboard data matching the original Angular structure
        operations_data = {
            "machine_id": vm.id,
            "machine_status": "Online" if vm.status == DeviceStatus.ONLINE else "Offline",
            "last_updated": datetime.utcnow().isoformat(),
            "cap_position": {
        ...
        # Cache the result
        if self.ops_cache:
            await self.ops_cache.cache_ice_cream_machine_operations(machine_id, operations_data)
    ...
    async def get_fleet_operations(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "operational_percentage": (online_count / total_machines * 100) if total_machines > 0 else 0,
            "locations": locations,
            "last_updated": datetime.utcnow().isoformat()
        }
