...
class SQLSecurityValidator:
    ...
    def __init__(self):
        ...
        self.compiled_unsafe_patterns = [re.compile(pattern, re.IGNORECASE) for pattern in self.unsafe_patterns]
    ...
    def validate_query_params(self, params: List) -> bool:
        ...
        for param in params:
            ...
            if isinstance(param, str):
                ...
                if param.strip().endswith(';') and any(keyword in param.upper() for keyword in 
                                                    ['DROP', 'DELETE', 'INSERT', 'UPDATE', 'SELECT']):
                    return False
                
                # Check for attempts to comment out the rest of the query
                if '--' in param and any(keyword in param.upper() for keyword in 
                                        ['DROP', 'DELETE', 'INSERT', 'UPDATE', 'SELECT']):
                    return False
                
                # Check for attempts to break out and start a new query
                if ';' in param and re.search(r';\s*(SELECT|INSERT|UPDATE|DELETE|DROP)', param, re.IGNORECASE):
                    return False
