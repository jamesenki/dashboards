...
def generate_mock_pdf(template_id, model_id):
    ...
    custom_styles = {
        "CustomTitle": ParagraphStyle(
            "CustomTitle", parent=styles["Title"], fontSize=24, alignment=1
        ),
        "CustomSubtitle": ParagraphStyle(
            "CustomSubtitle", parent=styles["Title"], fontSize=18, alignment=1
        ),
        "CustomHeading": ParagraphStyle(
            "CustomHeading", parent=styles["Heading1"], fontSize=16
        ),
        "CustomNormal": ParagraphStyle(
            "CustomNormal", parent=styles["Normal"], fontSize=12
        ),
    }
    ...
    if template_id == "performance":
        ...
        description = (
            "Summary of model performance metrics over the selected time period."
        )
        ...
        description = (
            "Summary of alerts triggered for the selected model and time period."
        )
    else:
    ...
    # Add report metadata
    model_name = (
        "Water Heater Prediction Model"
        if "water" in model_id
        else "Anomaly Detection Model"
    )
        ...
        TableStyle(
            [
                ...
                (
                    ...
                    (2, 1),
                    (2, -1),
                    lambda x, y: colors.red if "-" in data[y][2] else colors.green,
                ),
            ]
        )
    )
...
def create_dashboard_api(monitoring_service: ModelMonitoringService = None) -> FastAPI:
        ...
        if path.endswith("/models") or path.endswith("/models/archived"):
            ...
            if path.endswith("/models/archived"):
                ...
                mock_models = [
                    ...
                    if model["id"] == "maintenance-predictor-1"
                ]
            else:
            ...
            return JSONResponse(status_code=200, content=mock_models)
    ...
    async def get_model_versions(
        ...
        versions, is_mock = await app.state.monitoring_service.get_model_versions(
            model_id
        )
        return {"versions": versions, "is_mock_data": is_mock}
    ...
    async def get_model_metrics(
        ...
        try:
            ...
            metrics, is_mock = await app.state.monitoring_service.get_latest_metrics(
    ...
    async def apply_batch_operation(request_data: dict):
        ...
        params = {
            k: v for k, v in request_data.items() if k not in ["models", "operation"]
        }
        ...
        return await app.state.monitoring_service.apply_batch_operation(
            models, operation, params
        )

    @app.get(
        "/models/{model_id}/versions/{model_version}/metrics/{metric_name}/history"
    )
    ...
    @app.post(
        "/models/{model_id}/versions/{model_version}/alerts/rules", status_code=201
    )
    async def create_alert_rule(
        ...
        return {"id": rule_id, "status": "success", "is_mock_data": is_mock}
    ...
    async def get_alert_rules(
        ...
        try:
            ...
            rules, is_mock = await app.state.monitoring_service.get_alert_rules(
                model_id, model_version
            )
    ...
    async def get_triggered_alerts(
        ...
        try:
            ...
            if not isinstance(alerts, list):
                logger.warning(
                    f"get_triggered_alerts returned non-list: {type(alerts)}"
                )
                alerts = []
    ...
    async def get_all_alerts():
        ...
        try:
            ...
            for row in rows:
                ...
                try:
                    ...
                    formatted_alert = {
                        ...
                        "metric": alert_data.get(
                            "metric_name", "unknown"
                        ),  # Frontend expects 'metric' not 'metric_name'
                        "threshold": float(alert_data.get("threshold", 0.0))
                        if alert_data.get("threshold") is not None
                        else 0.0,
                        "value": float(
                            alert_data.get("metric_value", 0.0)
                        ),  # Frontend expects 'value' not 'current_value'
                        "severity": str(
                            alert_data.get("severity", "MEDIUM")
                        ).lower(),  # Frontend expects lowercase severity
                        "timestamp": alert_data.get(
                            "created_at", ""
                        ),  # Frontend expects 'timestamp' not 'triggered_at'
                        "status": "active"
                        if not alert_data.get("resolved", False)
                        else "resolved",  # Required by frontend
                    }
            ...
            if not alerts:
                ...
                logger.info(
                    "Direct query returned no alerts, trying fallback to model-specific alerts"
                )
                ...
                try:
                    ...
                    for model_id in all_models:
                        try:
                            (
                                model_alerts,
                                _,
                            ) = await app.state.monitoring_service.get_triggered_alerts(
                                model_id, "1.0"
                            )
                            fallback_alerts.extend(model_alerts)
                        except Exception as e:
                            logger.warning(
                                f"Error getting alerts for model {model_id}: {str(e)}"
                            )
                    ...
                    logger.error(
                        f"Error in fallback alerts retrieval: {str(fallback_error)}"
                    )
    ...
    async def create_tag(tag_data: dict):
        ...
        return app.state.monitoring_service.create_tag(
            name=tag_data.get("name"), color=tag_data.get("color", "blue")
        )
    ...
    async def update_tag(
        ...
        return app.state.monitoring_service.update_tag(
            tag_id=tag_id, name=tag_data.get("name"), color=tag_data.get("color")
        )
