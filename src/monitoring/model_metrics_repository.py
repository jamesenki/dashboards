...
class ModelMetricsRepository:
    ...
    def __init__(
        ...
        self.fallback_enabled = config.get_bool(
            "services.monitoring.fallback_to_mock", True
        )
        ...
        if os.environ.get("USE_MOCK_DATA", "False").lower() in ("true", "1", "t"):
            ...
            logging.info(
                "Using mock data for model metrics (set by environment variable)"
            )
        ...
        if not self.use_mock_data and sql_repo is None:
            try:
                ...
                if self.fallback_enabled:
                    logging.warning(
                        f"Failed to initialize database repository: {e}. Falling back to mock data."
                    )
                    self.use_mock_data = True
                else:
                    logging.error(
                        f"Failed to initialize database repository and fallback is disabled: {e}"
                    )
                    raise
        else:
        ...
        self.test_mode = test_mode or os.environ.get("TESTING", "False").lower() in (
            "true",
            "1",
            "t",
        )
    ...
    async def record_model_metrics(
        ...
        if self._should_use_mock_data():
            metric_id = self._mock_record_model_metrics(
                model_id, model_version, metrics, timestamp
            )
            return metric_id, True

        try:
            metric_id = await self.sql_repo.record_model_metrics(
                model_id, model_version, metrics, timestamp
            )
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for record_model_metrics"
                )
                metric_id = self._mock_record_model_metrics(
                    model_id, model_version, metrics, timestamp
                )
    ...
    async def get_model_metrics_history(
        ...
        try:
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for get_model_metrics_history"
                )
                metrics = self._mock_get_model_metrics_history(
                    model_id, model_version, metric_name, start_date, end_date
                )
    ...
    def _mock_get_model_metrics_history(
        ...
        if (
            metric_name is None
            and model_id in self.mock_metrics_history
            and model_version in self.mock_metrics_history[model_id]
        ):
            # Return all metrics for all metric names
            for metric_name_key, entries in self.mock_metrics_history[model_id][
                model_version
            ].items():
        ...
        # If metric_name is specified, return only that metric
            ...
            model_id in self.mock_metrics_history
            and model_version in self.mock_metrics_history[model_id]
            and metric_name in self.mock_metrics_history[model_id][model_version]
        ):
            for entry in self.mock_metrics_history[model_id][model_version][
                metric_name
            ]:
                if start_date <= entry["timestamp"] <= end_date:
                    result.append(
    ...
    async def get_latest_metrics(
        ...
        try:
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for get_latest_metrics"
                )
                metrics = self._mock_get_latest_metrics(model_id, model_version)
                return metrics, True
            else:
    ...
    def _mock_get_latest_metrics(
        ...
        if (
            ...
            for metric_name, history in self.mock_metrics_history[model_id][
                model_version
            ].items():
    ...
    def _should_use_mock_data(self) -> bool:
        ...
        if current_value.lower() in ("true", "1", "t"):
            logger.info(
                f"Using mock data for model metrics (USE_MOCK_DATA={current_value})"
            )
            return True
    ...
    async def get_archived_models(self) -> Tuple[List[Dict[str, Any]], bool]:
        ...
        if self._should_use_mock_data():
            logger.info(
                "Using mock data for get_archived_models due to USE_MOCK_DATA setting"
            )
            # Get only archived models from mock data
            archived_models = [
                model for model in self.mock_models if model.get("archived", False)
            ]
            # Return mock data with mock indicator flag
        ...
        try:
            ...
            if self.test_mode:
                # In test mode, just log a brief message
                logger.info(
                    f"Database error in get_archived_models during test: {str(e)}"
                )
            else:
                # In production, log the full error
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for get_archived_models"
                )
                # Get only archived models from mock data
                archived_models = [
                    model for model in self.mock_models if model.get("archived", False)
    ...
    async def get_alert_rules(
        ...
        if self._should_use_mock_data():
            logger.info(
                "Using mock data for get_alert_rules due to USE_MOCK_DATA setting"
            )
        ...
        try:
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for get_alert_rules"
                )
                rules = self._mock_get_alert_rules(model_id)
                return rules, True
            else:
    ...
    def _mock_get_alert_rules(self, model_id: str = None) -> List[Dict[str, Any]]:
        """Mock implementation for getting alert rules."""
        if model_id:
            return [
                rule for rule in self.mock_alert_rules if rule["model_id"] == model_id
            ]
        else:
            return self.mock_alert_rules

    def _add_data_source_to_models(
        self, models: List[Dict[str, Any]], source: str
    ) -> List[Dict[str, Any]]:
        """
    ...
    async def create_alert_rule(
        ...
        try:
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for create_alert_rule"
                )
                rule_id = self._mock_create_alert_rule(
                    model_id=model_id,
                    metric_name=metric_name,
    ...
    async def delete_alert_rule(self, rule_id: str) -> Tuple[bool, bool]:
        ...
        try:
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for delete_alert_rule"
                )
                success = self._mock_delete_alert_rule(rule_id)
                return success, True
            else:
    ...
    def _mock_delete_alert_rule(self, rule_id: str) -> bool:
        ...
        self.mock_alert_rules = [
            rule for rule in self.mock_alert_rules if rule["id"] != rule_id
        ]
    ...
    async def record_alert_event(
        ...
        """
        if self._should_use_mock_data():
            event = self._mock_record_alert_event(
        ...
        try:
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for record_alert_event"
                )
                event = self._mock_record_alert_event(
                    rule_id, model_id, metric_name, metric_value, severity
                )
    ...
    async def get_model_versions(
        ...
        if self._should_use_mock_data():
            logger.info(
                f"Using mock data for get_model_versions due to USE_MOCK_DATA setting"
            )
        ...
        try:
            ...
            if self.test_mode:
                # In test mode, just log a brief message
                logger.info(
                    f"Database error in get_model_versions during test: {str(e)}"
                )
            else:
                # In production, log the full error
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for get_model_versions"
                )
                versions = self._mock_get_model_versions(model_id)
                return versions, True
            else:
    ...
    async def get_triggered_alerts(
        ...
        if self._should_use_mock_data():
            logger.info(
                f"Using mock data for get_triggered_alerts due to USE_MOCK_DATA setting"
            )
        ...
        try:
            ...
            if self.test_mode:
                # In test mode, just log a brief message
                logger.info(
                    f"Database error in get_triggered_alerts during test: {str(e)}"
                )
            else:
                # In production, log the full error
            ...
            if self.fallback_enabled:
                logger.warning(
                    "Falling back to mock implementation for get_triggered_alerts"
                )
                alerts = self._mock_get_triggered_alerts(model_id, model_version)
                return alerts, True
            else:
