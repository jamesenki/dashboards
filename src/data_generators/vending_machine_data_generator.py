...
from src.models.vending_machine import (
    VendingMachine, VendingMachineReading, ProductItem,
    VendingMachineStatus, VendingMachineMode, LocationType, 
    UseType, SubLocation
)
...
class VendingMachineDataGenerator:
    ...
    PRODUCT_NAMES = [
        "Polar Delight Classic", "Polar Delight Zero", "Polar Delight Cherry",
        "Polar Delight Lime", "Mountain Mist Original", "Mountain Mist Lemon",
        "Fizzy Pop Cola", "Fizzy Pop Diet", "Energy Surge", "Energy Surge Max",
        "Aqua Pure Water", "Chocolate Bliss", "Caramel Dream", "Nutty Delight",
        "Cheesy Crunch", "Savory Snack Mix", "Fruit Chews", "Sweet Cookies"
    ]
    
    PRODUCT_CATEGORIES = [
        "Beverages", "Snacks", "Candy", "Chips", "Chocolate", "Healthy Options"
    ]
    
    # Business names for locations
    BUSINESS_NAMES = [
        "Central Hospital", "City College", "Metro Station", "Office Tower",
        "Shopping Mall", "Community Center", "Industrial Park", "Tech Campus",
        "Fitness Center", "Public Library", "Convention Center", "Airport Terminal",
        "Business Park", "Medical Center", "Recreation Facility"
    ]
    ...
    def generate_vending_machine(
        ...
        self._track_machine_properties(vending_machine.id, {
    ...
    def generate_vending_machines(self, count: int = 5, usage_patterns: Optional[List[str]] = None) -> List[VendingMachine]:
        """
    ...
    def generate_vending_machine_readings(
        ...
        if usage_pattern is None:
            ...
            stored_pattern = self._get_machine_property(vending_machine.id, 'usage_pattern')
            if stored_pattern:
                ...
                elif vending_machine.location_type in [LocationType.OFFICE, LocationType.SCHOOL]:
                    usage_pattern = "business_hours"
                elif vending_machine.location_type in [LocationType.TRANSPORTATION, LocationType.RETAIL]:
        ...
        if (end_time - start_time).total_seconds() > 24 * 3600:  # If period > 24 hours
            ...
            for _ in range(num_events):
                event_time = start_time + timedelta(seconds=self.rng.uniform(0, (end_time - start_time).total_seconds()))
        ...
        while current_time <= end_time:
            ...
            if door_status == "OPEN":
                temperature += self.rng.uniform(1.0, 3.0)  # Temperature rises when door is open
            
            # Power consumption varies with usage and door status
            power_consumption = base_power * hour_factor
            if door_status == "OPEN":
                power_consumption *= 0.8  # Reduced when door is open (cooling less effective)
            ...
            if hour_factor > 0.3:  # Some activity
                ...
                if sales_count > 0:
                    current_cash += sales_count * self.rng.uniform(1.5, 3.5)  # Average sale value
                    total_sales += sales_count
            
            # Create the reading
            reading = VendingMachineReading(
                timestamp=current_time,
                temperature=round(temperature, 1) if self.rng.random() > 0.05 else None,  # Occasionally missing temperature
                power_consumption=round(power_consumption, 1) if self.rng.random() > 0.08 else None,  # Occasionally missing power
                door_status=door_status if self.rng.random() > 0.02 else None,  # Rarely missing door status
                cash_level=round(current_cash, 2) if self.rng.random() > 0.1 else None,  # Sometimes missing cash level
    ...
    def apply_readings_to_vending_machine(self, vending_machine: VendingMachine, readings: List[VendingMachineReading]) -> None:
        """
        ...
        for reading in sorted_readings:
            ...
            if reading.sales_count and reading.sales_count > 0 and vending_machine.products:
                # Distribute sales across products
                self._update_product_quantities(vending_machine, reading.sales_count)
                
    def _update_product_quantities(self, vending_machine: VendingMachine, num_sales: int) -> None:
        """
        ...
        for _ in range(num_sales):
            ...
            for product in available_products:
                ...
                weight = 0.7 * (product.quantity / max(p.quantity for p in available_products)) + 0.3 * position_weight
                weights.append(weight)
            
            # Select a product to decrement
            selected_idx = self.rng.choices(range(len(available_products)), weights=weights, k=1)[0]
    ...
    def _update_machine_status(self, vending_machine: VendingMachine) -> None:
        ...
        if num_empty > len(vending_machine.products) * 0.3 or num_low > len(vending_machine.products) * 0.5:
            vending_machine.machine_status = VendingMachineStatus.NEEDS_RESTOCK
        else:
    ...
    def add_products_to_vending_machine(
        ...
        for product in products:
            ...
            if stock_level == "full":
                ...
                product.quantity = 0 if self.rng.random() < 0.3 else self.rng.randint(min_qty, min(3, max_qty))
            else:  # normal
    ...
    def add_maintenance_history(
        ...
        if maintenance_dates:
            ...
            next_maintenance = vending_machine.last_maintenance_date + timedelta(days=self.rng.randint(90, 180))
            vending_machine.next_maintenance_date = next_maintenance
            ...
            if vending_machine.next_maintenance_date < datetime.now():
                vending_machine.machine_status = VendingMachineStatus.MAINTENANCE_REQUIRED
    
    def save_to_file(self, vending_machines: List[VendingMachine], filename: str) -> None:
        """
    ...
    def _track_machine_properties(self, machine_id: str, properties: Dict[str, Any]) -> None:
        """
    ...
    def _get_machine_property(self, machine_id: str, property_name: str, default_value: Any = None) -> Any:
        """
        ...
        for vm_data in vending_machines_json:
            ...
            for key in ['last_maintenance_date', 'next_maintenance_date']:
                if key in vm_data and vm_data[key]:
                    vm_data[key] = datetime.fromisoformat(vm_data[key].replace('Z', '+00:00'))
