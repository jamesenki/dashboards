...
class WaterHeaterDataGenerator:
    ...
    def generate_water_heater(
        self, id: Optional[str] = None, usage_pattern: str = "commercial"
    ) -> WaterHeater:
        """
        ...
        current_temp = target_temp + self.rng.uniform(
            -5, 5
        )  # Random deviation from target
        ...
        water_heater = WaterHeater(
            ...
            efficiency_rating=round(
                self.rng.uniform(efficiency_range[0], efficiency_range[1]), 2
            ),
    ...
    def generate_water_heaters(
        ...
        for i in range(count):
            ...
            water_heater = self.generate_water_heater(
                id=f"wh-{i+1:04d}", usage_pattern=usage_pattern
            )
    ...
    def generate_water_heater_readings(
        ...
        while current_time <= end_time:
            ...
            temperature = self._generate_temperature(
                base_temp, temp_variance, hour_factor, current_time
            )
            ...
            reading = WaterHeaterReading(
                ...
                else None,  # Occasional missing pressure
                energy_usage=round(energy_usage, 1)
                if self.rng.random() > 0.05
                else None,  # Rare missing energy
    ...
    def _generate_temperature(
        self, base_temp: float, variance: float, hour_factor: float, timestamp: datetime
    ) -> float:
        """
        ...
        return max(
            35, min(90, temperature)
        )  # Ensure temperature stays in realistic range

    def apply_readings_to_water_heater(
        self, water_heater: WaterHeater, readings: List[WaterHeaterReading]
    ) -> None:
        """
