...
class ModelRegistry:
    ...
    def register_model(
        ...
        """
        model_id = f"{prediction_type}-{version}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
    ...
    def list_models(self, prediction_type: Optional[str] = None) -> List[Dict[str, Any]]:
        """
        ...
        for filename in os.listdir(self.registry_path):
            if filename.endswith(".json"):
                ...
                if prediction_type is None or metadata.get("prediction_type") == prediction_type:
...
class ModelMonitoring:
    ...
    def log_ground_truth(
        ...
        truth_file = os.path.join(truth_dir, f"{datetime.now().strftime('%Y%m%d')}.jsonl")
...
class MLOpsService:
    ...
    def track_prediction(
        ...
        """
        prediction_id = f"{model_id}-{device_id}-{datetime.now().strftime('%Y%m%d%H%M%S')}"
