...
class UsagePatternPredictor:
    ...
    def __init__(self):
        ...
        self._usage_thresholds = {
            "light": {"daily_usage_liters": 100, "heating_cycles_per_day": 4},
            "normal": {"daily_usage_liters": 200, "heating_cycles_per_day": 8},
            "heavy": {"daily_usage_liters": 250, "heating_cycles_per_day": 10},
        }

        # Define component wear factors based on usage
        self._component_wear_factors = {
            "heating_element": {"light": 0.7, "normal": 1.0, "heavy": 1.3},
            "thermostat": {"light": 0.8, "normal": 1.0, "heavy": 1.2},
            "anode_rod": {"light": 0.85, "normal": 1.0, "heavy": 1.25},
            "tank_integrity": {"light": 0.9, "normal": 1.0, "heavy": 1.15},
        }
    ...
    async def predict(
        ...
        impact_factors = [
            ...
            for component, component_data in result.raw_details[
                "impact_on_components"
            ].items()
        ]
    ...
    def _classify_usage(self, features: Dict[str, Any]) -> str:
        ...
        if (
            avg_daily_usage >= self._usage_thresholds["heavy"]["daily_usage_liters"]
            or avg_heating_cycles
            >= self._usage_thresholds["heavy"]["heating_cycles_per_day"]
        ):
            usage_level = "heavy"
        # Check for light usage
        elif (
            avg_daily_usage <= self._usage_thresholds["light"]["daily_usage_liters"]
            and avg_heating_cycles
            <= self._usage_thresholds["light"]["heating_cycles_per_day"]
        ):
            usage_level = "light"
    ...
    def _get_average_daily_usage(self, features: Dict[str, Any]) -> float:
        ...
        if "daily_usage_liters" not in features:
            return self._usage_thresholds["normal"][
                "daily_usage_liters"
            ]  # Default if no data
    ...
    def _get_average_heating_cycles(self, features: Dict[str, Any]) -> float:
        ...
        """
        if "heating_cycles_per_day" in features and isinstance(
            features["heating_cycles_per_day"], (int, float)
        ):
            return float(features["heating_cycles_per_day"])

        if "heating_cycles_per_day" in features and isinstance(
            features["heating_cycles_per_day"], list
        ):
    ...
    def _analyze_usage_patterns(
        self, daily_usage: List[Dict], result: PredictionResult
    ) -> None:
        """
    ...
    def _calculate_component_impacts(
        ...
        for component in components:
            ...
            base_wear_factor = self._component_wear_factors[component].get(
                usage_classification, self._component_wear_factors[component]["normal"]
            )
    ...
    def _calculate_days_until_impact(
        self, component: str, wear_factor: float, usage_classification: str
    ) -> int:
        """
    ...
    def _project_efficiency_changes(
        self,
        usage_classification: str,
        features: Dict[str, Any],
        result: PredictionResult,
        ...
        base_decline_rates = {"light": 3, "normal": 5, "heavy": 8}
    ...
    def _generate_recommendations(self, result: PredictionResult) -> None:
        ...
        for component, impact in result.raw_details["impact_on_components"].items():
            ...
            if wear_factor > 1.1:
                ...
                action = RecommendedAction(
                    ...
                    + timedelta(days=min(days_until_impact // 2, 90)),
        ...
        if "efficiency_projections" in result.raw_details:
            decline_90 = result.raw_details["efficiency_projections"].get(
                "90_day_projected_decline", 0
            )
        ...
        if not result.recommended_actions:
            ...
            if usage_class == "light":
                action = RecommendedAction(
                    ...
                    due_date=datetime.now()
                    + timedelta(days=180),  # Semi-annual reminder
                    estimated_cost=0.0,
    ...
    def _generate_optimization_recommendations(self, result: PredictionResult) -> None:
        ...
        if "efficiency_projections" in result.raw_details:
            est_annual_cost_increase = result.raw_details["efficiency_projections"].get(
                "estimated_annual_cost_increase", 100
            )
