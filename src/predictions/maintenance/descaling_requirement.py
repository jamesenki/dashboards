...
class DescalingRequirementPrediction(IPredictionModel):
    ...
    async def predict(
        ...
        descaling_requirement = self._calculate_descaling_requirement(
            scale_estimate, days_since_descaling, efficiency_metrics
        )
    ...
    def _prepare_telemetry_data(self, features: Dict[str, Any]) -> pd.DataFrame:
        ...
        try:
            ...
            df_data = {"timestamp": features.get("timestamp", [])}
    ...
    def _days_since_last_descaling(self, features: Dict[str, Any]) -> int:
        ...
        for maintenance in maintenance_history:
            ...
            if maintenance_date:
                if maintenance_type == "installation" and (
                    installation_date is None or maintenance_date > installation_date
                ):
                    installation_date = maintenance_date
                elif maintenance_type == "descaling" and (
                    last_descaling_date is None
                    or maintenance_date > last_descaling_date
                ):
    ...
    def _calculate_efficiency_metrics(
        ...
        if "efficiency" in telemetry_df.columns and len(telemetry_df) >= 3:
            ...
            metrics["efficiency_trend"] = self._calculate_trend(
                telemetry_df["efficiency"]
            )
            ...
            if len(efficiencies) >= 2:
                ...
                days_span = (
                    telemetry_df["timestamp"].max() - telemetry_df["timestamp"].min()
                ).days
        ...
        if "flow_rate" in telemetry_df.columns and len(telemetry_df) >= 3:
            ...
            first_flow = np.mean(
                telemetry_df["flow_rate"].iloc[: min(3, len(telemetry_df))]
            )
            last_flow = np.mean(
                telemetry_df["flow_rate"].iloc[-min(3, len(telemetry_df)) :]
            )
        ...
        if "heating_cycles" in telemetry_df.columns and len(telemetry_df) >= 3:
            # Get first and last values
            first_cycle = np.mean(
                telemetry_df["heating_cycles"].iloc[: min(3, len(telemetry_df))]
            )
            last_cycle = np.mean(
                telemetry_df["heating_cycles"].iloc[-min(3, len(telemetry_df)) :]
            )
    ...
    def _estimate_scale_buildup(
        ...
        estimate["thickness_mm"] = (
            (days_since_descaling / 365) * hardness_factor * 5.0
        )  # Max 5mm per year with hardest water
        ...
        estimate["volume_percentage"] = (
            estimate["thickness_mm"] * 3
        )  # Approximate relationship
        estimate["surface_coverage"] = min(
            100, estimate["thickness_mm"] * 10
        )  # Approximate percentage coverage
    ...
    def _calculate_descaling_requirement(
        ...
        base_probability = min(
            1.0, thickness / 5.0
        )  # Scale of 5mm or more requires descaling

        # Adjust based on days since last descaling
        time_factor = min(
            1.0, days_since_descaling / 365
        )  # Consider annual descaling baseline
        ...
        combined_probability = (
            base_probability * 0.5 + time_factor * 0.2 + efficiency_factor * 0.3
        )
    ...
    def _calculate_confidence(
        ...
        missing_features = [
            f for f in key_features if f not in features or not features[f]
        ]
    ...
    def _generate_recommendations(
        ...
        if water_hardness > 180 and severity in [
            ActionSeverity.HIGH,
