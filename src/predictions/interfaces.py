...
class ModelRegistry:
    ...
    def register_model(
        self, prediction_type: str, model_version: str, model: IPredictionModel
    ) -> None:
        """
    ...
    def get_model(
        self, prediction_type: str, model_version: Optional[str] = None
    ) -> IPredictionModel:
        """
        ...
        if prediction_type not in self._models:
            raise ValueError(
                f"No models registered for prediction type: {prediction_type}"
            )
        ...
        if not models:
            raise ValueError(
                f"No models available for prediction type: {prediction_type}"
            )

        if model_version is not None:
            if model_version not in models:
                raise ValueError(
                    f"Model version {model_version} not found for prediction type: {prediction_type}"
                )
...
class ActionRegistry:
    ...
    def register_recommender(
        self, prediction_type: str, recommender: IActionRecommender
    ) -> None:
        """
    ...
    def get_recommender(self, prediction_type: str) -> IActionRecommender:
        ...
        if prediction_type not in self._recommenders:
            raise ValueError(
                f"No action recommender registered for prediction type: {prediction_type}"
            )
