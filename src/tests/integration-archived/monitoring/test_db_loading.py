...
class TestDatabaseLoading:
    ...
    async def test_mock_data_is_fallback_only(self):
        ...
        assert (
            unique_model is None
        ), "Found unique test model in fallback data - still using database"
    ...
    async def test_toggle_between_db_and_mock(self):
        ...
        unique_model = next(
            (m for m in models_from_db if m["id"] == self.unique_model_id), None
        )
        ...
        unique_model = next(
            (m for m in models_from_mock if m["id"] == self.unique_model_id), None
        )
    ...
    async def test_record_and_retrieve_metrics(self):
        ...
        await self.service.record_model_metrics(
            model_id=self.unique_model_id, model_version="1.0", metrics=test_metrics
        )

        # Now retrieve the metrics history for one of these metrics
        history = await self.service.get_model_metrics_history(
            model_id=self.unique_model_id, model_version="1.0", metric_name="precision"
        )
        ...
        if "value" in history[0]:
            ...
            raise KeyError(
                f"Expected 'value' or 'metric_value' in history entries, found: {available_keys}"
            )
