...
class TestReportsPage(unittest.TestCase):
    ...
    def test_reports_page_template_extends_base(self):
        ...
        try:
            with open(
                "/Users/lisasimon/repos/IoTSphereAngular/IoTSphere-Refactor/frontend/templates/model-monitoring/reports.html",
                "r",
            ) as f:
                html_content = f.read()
        except FileNotFoundError:
            self.skipTest(
                "Reports page template not yet created - this is normal for TDD"
            )
    ...
    def test_reports_page_navigation(self):
        ...
        self.assertIn(
            '<a href="/model-monitoring/reports" class="active">', html_content
        )
    ...
    def test_url_parameter_handling(self):
        ...
        test_params = (
            "?template=performance&model=model1&startDate=2025-03-01&endDate=2025-03-31"
        )
        ...
        self.assertEqual(
            ...
            200,
            f"Failed to handle URL parameters: {response.text}",
        )
    ...
    def test_export_pdf_endpoint(self):
        ...
        pdf_response = self.client.post(
            "/api/monitoring/reports/export/pdf",
            json={
    ...
    def test_export_csv_endpoint(self):
        ...
        csv_response = self.client.post(
            "/api/monitoring/reports/export/csv",
            json={
                "template_id": "performance",
                "model_id": "model1",
                "start_date": "2025-03-01",
    ...
    def test_reports_page_has_dark_theme(self):
        ...
        dark_theme_found = any(
            indicator in html_content for indicator in dark_theme_indicators
        )
        self.assertTrue(
            dark_theme_found, "Dark theme styling not found in reports page template"
        )
    ...
    def test_end_to_end_report_generation(self):
        ...
        self.assertEqual(
            models_response.status_code, 200, "Models endpoint should return 200 OK"
        )
        ...
        generate_response = self.client.post(
            "/api/monitoring/reports/generate", json=report_request
        )
        self.assertEqual(
            generate_response.status_code, 200, "Report generation should return 200 OK"
        )
        ...
        self.assertIsInstance(
            report["metrics"], list, "Report metrics should be a list"
        )
        self.assertTrue(
            len(report["metrics"]) > 0, "Report should contain at least one metric"
        )
