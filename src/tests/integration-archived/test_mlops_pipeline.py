...
class TestMLOpsPipelineIntegration:
    ...
    def setup_method(self):
        ...
        self.model_registry = ModelRegistry(
            db=self.db_mock, storage_path=self.test_models_dir
        )
        self.feedback_service = FeedbackService(
            db=self.db_mock, model_registry=self.model_registry
        )
    ...
    def test_feature_to_prediction_flow(self):
        ...
        with patch("os.path.exists", return_value=True):
            with patch("shutil.copy2"):
                ...
                # Activate the model
                self.model_registry.activate_model(
                    "component_failure", "2025.03.27.001"
                )
                ...
                self.model_registry.get_active_model = MagicMock(
