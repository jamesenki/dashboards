...
from src.ml.models.maintenance_prediction import MaintenancePredictionModel, MaintenancePrediction
...
def sample_test_data():
    ...
    for i in range(1000):
        if maintenance_needed[i] == 1:
            ...
            data['temperature'][i] = max(data['temperature'][i], np.random.normal(8, 1.5))
            # Machines needing maintenance tend to have higher vibration
            data['vibration_level'][i] = max(data['vibration_level'][i], np.random.normal(0.8, 0.1))
            # Machines needing maintenance tend to have longer compressor runtime
            data['compressor_runtime'][i] = max(data['compressor_runtime'][i], np.random.normal(170, 20))
...
def test_test_data_quality(sample_test_data):
    ...
    assert 0 < sample_test_data['temperature'].mean() < 10, "Temperature values outside expected range"
    assert 0 < sample_test_data['power_consumption'].mean() < 200, "Power consumption outside expected range"
