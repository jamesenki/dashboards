...
class TestDBFirstWithMockIndicators:
    ...
    def _add_repository_spies(self):
        ...
        self.mock_models_spy = MagicMock(wraps=self.metrics_repository._mock_record_model_metrics)
    ...
    def _seed_test_data(self):
        ...
        self.db.execute(
            ...
            (self.model_id, "DB-First Test Model", "Testing DB-first with indicators", 0)
        )
        ...
        for metric_name, metric_value in metrics_data.items():
            self.db.execute(
                ...
                (str(uuid.uuid4()), self.model_id, self.model_version, metric_name, metric_value)
            )
        ...
        self.db.execute(
            """INSERT INTO alert_rules 
               ...
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (str(uuid.uuid4()), self.model_id, self.model_version, "Test Rule", "accuracy", 0.9, "BELOW", "WARNING", "Test description", 1)
        )
    ...
    async def test_all_repository_methods_return_indicators(self):
        ...
        versions, is_mock = await self.metrics_repository.get_model_versions(self.model_id)
        assert is_mock is False
        ...
        with patch.object(self.sql_repo_spy, 'get_model_versions', 
                         side_effect=Exception("DB failure")):
            versions, is_mock = await self.metrics_repository.get_model_versions(self.model_id)
            assert is_mock is True
        ...
        with patch.object(self.sql_repo_spy, 'get_triggered_alerts', 
                         side_effect=Exception("DB failure")):
            alerts, is_mock = await self.metrics_repository.get_triggered_alerts(
                self.model_id, self.model_version)
            assert is_mock is True
    ...
    async def test_service_propagates_mock_indicators(self):
        ...
        # Test mock fallback
        with patch.object(self.metrics_repository, 'get_models', 
    ...
    def test_api_endpoints_include_mock_indicators(self):
        ...
        with patch.object(self.service, 'get_monitored_models', 
                         return_value=([{'id': 'model1'}], False)), \
             patch.object(self.service, 'get_latest_metrics', 
                         return_value=({'accuracy': 0.95}, False)), \
             patch.object(self.service, 'get_alert_rules', 
                         return_value=([{'id': 'rule1'}], False)), \
             patch.object(self.service, 'get_triggered_alerts', 
    ...
    def test_api_endpoints_indicate_mock_on_db_failure(self):
        ...
        with patch.object(self.service, 'get_models', 
                         return_value=([{"id": "mock1"}], True)):
        ...
        with patch.object(self.service, 'get_model_metrics_history', 
        ...
        with patch.object(self.service, 'get_alert_rules', 
        ...
        with patch.object(self.service, 'get_triggered_alerts', 
    ...
    async def test_create_operations_return_indicators(self):
        ...
        with patch.object(self.metrics_repository, 'create_alert_rule', 
                         side_effect=Exception("DB failure")):
