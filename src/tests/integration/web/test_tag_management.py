...
class TestTagManagement(unittest.TestCase):
    ...
    def test_create_tag(self):
        ...
        new_tag_data = {
            "name": f"new-test-tag-{uuid.uuid4()}",
            "color": "red"
        }
    ...
    def test_update_tag(self):
        ...
        # Prepare update data
        update_data = {
            "name": f"updated-tag-{uuid.uuid4()}",
            "color": "green"
        }
        
        # Update the tag
        response = self.client.put(f"/api/monitoring/tags/{self.test_tag_id}", json=update_data)
    ...
    def test_delete_tag(self):
        ...
        delete_tag_data = {
            "name": f"delete-test-tag-{uuid.uuid4()}",
            "color": "purple"
        }
