...
class TestModelsTab(unittest.TestCase):
    ...
    def setUp(self):
        ...
        with open(
            "/Users/lisasimon/repos/IoTSphereAngular/IoTSphere-Refactor/frontend/templates/model-monitoring/models.html",
            "r",
        ) as f:
    ...
    def test_models_table_structure(self):
        ...
        for header in expected_headers:
            self.assertTrue(
                any(header in th.text for th in table_headers),
    ...
    def test_fleet_metrics_display(self):
        ...
        for metric in expected_metrics:
            self.assertTrue(
                any(
                    metric in div.text
                    for div in fleet_metrics.find_all("div", class_="metric-card")
                ),
    ...
    def test_health_indicators(self):
        ...
        self.assertGreater(
            len(health_indicators), 0, "Health indicators should be present"
        )
    ...
    def test_model_filtering(self):
        ...
        for filter_option in expected_filters:
            self.assertTrue(
                any(
                    filter_option in div.text for div in filter_section.find_all("div")
                ),
    ...
    def test_batch_operations(self):
        ...
        for operation in expected_operations:
            self.assertTrue(
                any(
                    operation in button.text for button in batch_ops.find_all("button")
                ),
    ...
    def test_model_selection(self):
        ...
        checkboxes = self.soup.find_all(
            "input", {"type": "checkbox", "class": "model-select"}
        )
        self.assertGreater(
            len(checkboxes), 0, "Checkboxes for model selection should be present"
        )

        # Check for 'select all' checkbox
        select_all = self.soup.find(
    ...
    def test_inventory_management(self):
        ...
        self.assertIsNotNone(
            inventory_section, "Inventory management section should exist"
        )
        ...
        for option in expected_options:
            self.assertTrue(
                any(option in div.text for div in inventory_section.find_all("div")),
    ...
    def test_javascript_functions(self):
        ...
        for function in expected_functions:
            self.assertIn(
                function,
                script_content,
                f"JavaScript should include {function} function",
            )
    ...
    def test_all_ui_components(self):
        ...
        nav_section = html_content[
            html_content.find("{% block nav %}") : html_content.find(
                "{% endblock %}", html_content.find("{% block nav %}")
            )
        ]

        for link_text in expected_links:
            self.assertIn(
                link_text, nav_section, f"Navigation should include link to {link_text}"
            )
        ...
        for select_id in expected_selects:
            self.assertIn(
                select_id,
                select_ids,
                f"Page should have select element with id {select_id}",
            )
        ...
        button_texts = [
            button.text.strip() for button in buttons if button.text.strip()
        ]
        ...
        action_texts = [
            button.text.strip() for button in action_buttons if button.text.strip()
        ]

        for action in expected_actions:
            self.assertTrue(
                any(action in text for text in action_texts),
    ...
    def test_api_endpoints_in_javascript(self):
        ...
        expected_endpoints = ["/api/monitoring/models", "/api/monitoring/models/batch"]

        for endpoint in expected_endpoints:
            self.assertIn(
                endpoint,
                script_content,
                f"JavaScript should include API endpoint {endpoint}",
            )
    ...
    def test_batch_operations_api_consistency(self):
        ...
        self.assertRegex(
            script_content, post_pattern, "Batch operations should use POST method"
        )
    ...
    def test_archive_toggle_functionality(self):
        ...
        self.assertEqual(
            active_link.text,
            "View Archived",
            "Link should say 'View Archived' when viewing active models",
        )
        self.assertEqual(
            active_link["href"],
            "/model-monitoring/models?view=archived",
            "Link should point to the archived view URL",
        )

        # Test archived view
        archived_html = (
            html.replace("View Archived", "View Active")
            .replace("?view=archived", "")
            .replace("viewingArchived = false", "viewingArchived = true")
        )
        archived_document = BeautifulSoup(archived_html, "html.parser")
        archived_link = archived_document.select_one(".header-controls a")
        self.assertIsNotNone(
            archived_link, "Archive toggle link should exist in archived view"
        )
        self.assertEqual(
            archived_link.text,
            "View Active",
            "Link should say 'View Active' when viewing archived models",
        )
        self.assertEqual(
            archived_link["href"],
        ...
        self.assertIn(
            "/api/monitoring/models", html, "Active models API endpoint should be used"
        )
        self.assertIn(
            "/api/monitoring/models/archived",
            html,
            "Archived models API endpoint should be used",
        )
    ...
    def test_models_archive_toggle_url_param(self):
        ...
        self.assertEqual(
            toggle_link.text.strip(),
            "View Archived",
            "Toggle link text should be 'View Archived' in active view",
        )
        self.assertTrue(
            toggle_link["href"].endswith("?view=archived"),
    ...
    def test_model_action_buttons(self):
        ...
        self.assertGreater(
            len(view_buttons), 0, "At least one View button should exist"
        )
        self.assertGreater(
            len(edit_buttons), 0, "At least one Edit button should exist"
        )

        # Each button should have a data-model-id attribute
        for button in view_buttons:
            self.assertIsNotNone(
                button.get("data-model-id"),
    ...
    def test_batch_operation_buttons(self):
        ...
        for button_id, button in batch_buttons.items():
            self.assertTrue(
                button.has_attr("disabled") or "disabled" in button.get("class", []),
    ...
    def test_manage_tags_button(self):
        ...
        self.assertIn("loadTags", html, "loadTags function should exist in JavaScript")
