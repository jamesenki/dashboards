...
class TestSQLInjectionPrevention:
    ...
    def test_detect_unsafe_sql_query(self):
        ...
        for query in unsafe_queries:
            assert self.sql_validator.is_unsafe(query), f"Failed to detect unsafe query: {query}"
    ...
    def test_safe_sql_queries(self):
        ...
        for query in safe_queries:
            assert not self.sql_validator.is_unsafe(query), f"Incorrectly flagged safe query: {query}"
    ...
    def test_data_access_with_sql_injection_attempt(self):
        ...
        with pytest.raises(ValueError, match="SQL injection attempt detected"):
            self.data_access.execute("SELECT * FROM users WHERE id = ?", [malicious_param])
    ...
    def test_whitelist_query_patterns(self):
        ...
        self.sql_validator.add_whitelist_pattern(r"^SELECT \* FROM allowed_table WHERE id = \?$")
        
        # Act & Assert
        assert not self.sql_validator.is_unsafe("SELECT * FROM allowed_table WHERE id = ?")
