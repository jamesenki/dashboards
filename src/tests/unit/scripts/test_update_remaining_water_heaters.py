...
async def test_update_remaining_water_heaters():
    ...
    mock_device1.name = "Office Boiler Room - HeatMaster 200L"  # Commercial indicator in name
    mock_device1.properties = {"capacity": 100}  # No heater_type yet
    
    mock_device2 = MagicMock()
    mock_device2.id = "wh-test-2"
    mock_device2.name = "Apartment 101 Bathroom - WaterWarm 50L"  # No commercial indicator
    mock_device2.properties = {"capacity": 50}  # No heater_type yet
    
    mock_device3 = MagicMock()
    mock_device3.id = "wh-test-3"
    mock_device3.name = "Building A Utility Room - ThermoMax 150L"  # Commercial indicator
    ...
    mock_result.scalars.return_value.all.return_value = [mock_device1, mock_device2, mock_device3, mock_device4]
    ...
    with patch("src.scripts.update_remaining_water_heaters.get_db_session", return_value=mock_session_generator):
        with patch("src.scripts.update_remaining_water_heaters.random.random", return_value=0.4):  # > 0.3, so not commercial
    ...
    assert mock_device1.properties["specification_link"] == "/docs/specifications/water_heaters/commercial.md"
    ...
    assert mock_device2.properties["specification_link"] == "/docs/specifications/water_heaters/residential.md"
    ...
    assert "capacity" in mock_device4.properties  # Original property should still be there
...
async def test_main_execution():
    ...
    with patch("src.scripts.update_remaining_water_heaters.update_remaining_water_heaters") as mock_update:
