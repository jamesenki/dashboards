...
async def test_get_ice_cream_operations_from_cache(mock_repo, mock_cache, sample_vm):
    ...
    cached_ops = {
        ...
        "pod_code": "12345",
        "ice_cream_inventory": [
            {"name": "Vanilla", "current_level": 5, "max_level": 10}
        ],
    }
...
async def test_operations_update_invalidates_cache(mock_repo, mock_cache, sample_vm):
    ...
    mock_cache.invalidate_operations_cache.assert_called_once_with(
        "test-vm-1", "vending_machine"
    )

    # Verify update notification was published
    mock_cache.publish_operations_update.assert_called_once_with(
        "test-vm-1", "vending_machine"
    )
