...
class TestEnvironmentFileProvider(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.base_config = {
            ...
            "database": {"type": "sqlite", "path": "test.db", "fallback_to_mock": True},
        }
    ...
    def test_default_environment(self):
        ...
        self.assertEqual(
            provider.get("app.log_level"), "DEBUG"
        )  # Overridden by dev config
        self.assertEqual(
            provider.get("database.path"), "dev.db"
        )  # Overridden by dev config
    ...
    def test_production_environment(self):
        ...
        self.assertEqual(
            provider.get("app.environment"), "production"
        )  # Overridden by prod config
        self.assertEqual(
            provider.get("app.log_level"), "WARNING"
        )  # Overridden by prod config
        self.assertEqual(
            provider.get("database.type"), "postgres"
        )  # Overridden by prod config
        self.assertEqual(
            provider.get("database.host"), "localhost"
        )  # From prod config with default
        self.assertFalse(
            provider.get("database.fallback_to_mock")
        )  # Overridden by prod config
