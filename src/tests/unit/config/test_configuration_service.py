...
class TestConfigurationProviders:
    ...
    def test_environment_provider(self):
        ...
        try:
            ...
            os.environ["IOTSPHERE_API_CORS_ALLOWED_ORIGINS"] = "http://localhost:4200,http://example.com"
            ...
            assert provider.get("database.port") == "5434"  # Note: comes as string from env
            
            # Test list parsing
            assert provider.get("api.cors.allowed_origins") == "http://localhost:4200,http://example.com"
...
class TestConfigurationService:
    ...
    def test_typed_access(self):
        ...
        service = ConfigurationService()
        service.register_provider(DefaultProvider({
            "app": {"name": "IoTSphere", "debug": "true", "port": "8080", "workers": "4"},
    ...
    def test_section_access(self):
        ...
        # With default
        assert service.get_section("nonexistent", {"default": True}) == {"default": True}
