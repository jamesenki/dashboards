...
class TestLifespanEstimationPrediction:
    ...
    def new_device_features(self):
        ...
        return {
            "device_id": "wh123",
            "installation_date": (datetime.datetime.now() - datetime.timedelta(days=365)).isoformat(),
            "total_operation_hours": 3000,
            "usage_intensity": "normal",
            "water_hardness": 100,  # moderately hard water
            "temperature_settings": [55, 55, 60, 55, 55, 55, 55],  # mostly moderate temperature
    ...
    def aging_device_features(self):
        ...
        return {
            "device_id": "wh456",
            "installation_date": (datetime.datetime.now() - datetime.timedelta(days=2190)).isoformat(),  # 6 years
            "total_operation_hours": 18000,
            "usage_intensity": "heavy",
            "water_hardness": 180,  # hard water
            "temperature_settings": [65, 65, 70, 65, 65, 65, 65],  # higher temperature settings
            ...
            "maintenance_history": [
                {
                    ...
                    "date": (datetime.datetime.now() - datetime.timedelta(days=730)).isoformat(),  # 2 years ago
                    "findings": "moderate_corrosion"
                },
                {
                    "type": "inspection",
                    "date": (datetime.datetime.now() - datetime.timedelta(days=365)).isoformat(),  # 1 year ago
    ...
    def old_device_features(self):
        ...
        return {
            "device_id": "wh789",
            "installation_date": (datetime.datetime.now() - datetime.timedelta(days=3650)).isoformat(),  # 10 years
            "total_operation_hours": 30000,
            "usage_intensity": "heavy",
            "water_hardness": 250,  # very hard water
            "temperature_settings": [70, 70, 75, 70, 70, 70, 70],  # high temperature settings
            ...
            "maintenance_history": [
                {
                    ...
                    "date": (datetime.datetime.now() - datetime.timedelta(days=1095)).isoformat(),  # 3 years ago
                ...
                {
                    "type": "leak_repair",
                    "date": (datetime.datetime.now() - datetime.timedelta(days=180)).isoformat(),  # 6 months ago
                    "findings": "minor_leak"
                }
            ]
        }
    ...
    def minimal_device_features(self):
        ...
        return {
            "device_id": "wh999",
            "installation_date": (datetime.datetime.now() - datetime.timedelta(days=1825)).isoformat(),  # 5 years
            "total_operation_hours": 15000,
            "water_hardness": 150  # moderately hard water
        }
    
    async def test_lifespan_prediction_for_new_device(self, prediction_model, new_device_features):
        ...
        assert 0.7 <= result.predicted_value <= 0.95  # Should have high remaining lifespan
    ...
    async def test_lifespan_prediction_for_aging_device(self, prediction_model, aging_device_features):
        ...
        # Validate the prediction result
        assert isinstance(result, PredictionResult)
        assert 0.3 <= result.predicted_value <= 0.7  # Should have moderate remaining lifespan
        assert len(result.recommended_actions) > 0
        ...
        assert result.raw_details["estimated_remaining_years"] < 7
        assert 5 <= result.raw_details["current_age_years"] <= 6  # Should be around 6 years
    
    async def test_lifespan_prediction_for_old_device(self, prediction_model, old_device_features):
        ...
        # Validate the prediction result
        assert isinstance(result, PredictionResult)
        assert 0.05 <= result.predicted_value <= 0.35  # Should have low remaining lifespan
        assert len(result.recommended_actions) >= 2  # Should have multiple actions
        
        # Check for critical actions
        has_replacement_action = any(
            "replace" in action.description.lower() or 
            "replacement" in action.description.lower() 
    ...
    async def test_lifespan_prediction_with_minimal_data(self, prediction_model, minimal_device_features):
        ...
        has_inspection_action = any(
            "inspect" in action.description.lower() or 
            "assessment" in action.description.lower() 
            for action in result.recommended_actions
        )
        assert has_inspection_action
        
        # Should indicate that the prediction is based on limited data
        assert "limited data" in str(result.raw_details).lower() or "limited information" in str(result.raw_details).lower()
    ...
    async def test_missing_installation_date(self, prediction_model):
        ...
        has_data_quality_action = any(
            "data" in action.description.lower() and 
            "quality" in action.description.lower() 
            for action in result.recommended_actions
        )
