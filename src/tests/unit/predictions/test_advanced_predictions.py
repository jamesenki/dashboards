...
class TestAnomalyDetectionPredictor:
    ...
    def test_anomaly_detection_prediction(self):
        ...
        assert (
            "anomaly" in action.description.lower()
            or "pattern" in action.description.lower()
        )
    ...
    def test_temperature_trend_analysis(self):
        ...
        assert (
            "days_until_critical" in result.raw_details["trend_analysis"]["temperature"]
        )
...
class TestMultiFactorPredictor:
    ...
    def test_multi_factor_prediction(self):
        ...
        mock_features = {
            ...
            - timedelta(days=180),  # Maintenance history
        }
        ...
        assert (
            "days_until_critical"
            in result.raw_details["environmental_impact"]["scale_buildup"]
        )
    ...
    def test_environmental_impact_modeling(self):
        ...
        assert "scale_buildup" in result.raw_details["environmental_impact"]
        assert (
            result.raw_details["environmental_impact"]["scale_buildup"][
                "days_until_critical"
            ]
            < 60
        )  # Should predict rapid scaling
    ...
    def test_component_interaction_predictions(self):
        ...
        assert "component_interactions" in result.raw_details
        assert (
            "thermostat_heating_element" in result.raw_details["component_interactions"]
        )
        interaction = result.raw_details["component_interactions"][
            "thermostat_heating_element"
        ]
        assert "stress_increase_percent" in interaction
        assert (
            interaction["stress_increase_percent"] > 0
        )  # Should predict increased stress
