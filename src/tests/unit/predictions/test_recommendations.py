...
class TestRecommendationGeneration:
    ...
    def setup_method(self):
        ...
        self.standard_features = {
            ...
            'maintenance_history': [
                {'date': datetime.now() - timedelta(days=180), 'type': 'routine_check', 'notes': 'All systems normal'},
            ],
    ...
    async def test_anomaly_detection_always_provides_recommendations(self):
        ...
        for action in result.recommended_actions:
            assert action.description is not None and isinstance(action.description, str)
    ...
    async def test_all_raw_details_properly_formatted(self):
        ...
        for predictor in predictors:
            ...
            if 'environmental_impact' in result.raw_details:
                assert not isinstance(result.raw_details['environmental_impact'], object) or isinstance(result.raw_details['environmental_impact'], dict), \
            ...
            if 'component_interactions' in result.raw_details:
                assert isinstance(result.raw_details['component_interactions'], (list, dict)), \
        ...
        if 'environmental_impact' in lifespan_result.raw_details:
            assert not isinstance(lifespan_result.raw_details['environmental_impact'], object) or isinstance(lifespan_result.raw_details['environmental_impact'], dict)
        
        if 'component_interactions' in lifespan_result.raw_details:
            assert isinstance(lifespan_result.raw_details['component_interactions'], (list, dict))
