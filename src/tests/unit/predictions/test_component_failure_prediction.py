...
class TestComponentFailurePrediction:
    ...
    def component_failure_model(self):
        ...
        model._fetch_device_data_from_db = AsyncMock(
            return_value={
    ...
    @pytest.mark.asyncio
    async def test_component_failure_prediction(
        self, component_failure_model, sample_features
    ):
    ...
    async def test_action_recommendations(self, component_failure_recommender):
        ...
        heating_element_actions = [
            a for a in actions if "heating element" in a.description.lower()
        ]
        ...
        high_severity_actions = [
            a for a in actions if a.severity == ActionSeverity.HIGH
        ]
    ...
    async def test_prediction_model_with_low_risk(self):
        ...
        features = {
            ...
            "timestamp": pd.date_range(
                end=datetime.now(), periods=30, freq="h"
            ).tolist(),  # Use 'h' instead of 'H'
            "temperature": np.ones(30) * 65
            + np.random.normal(0, 0.5, 30),  # Stable temperature
            "pressure": np.ones(30) * 2.5
            + np.random.normal(0, 0.1, 30),  # Stable pressure
            "energy_usage": np.ones(30) * 1000
            + np.random.normal(0, 20, 30),  # Stable energy
            "flow_rate": np.ones(30) * 10
            + np.random.normal(0, 0.1, 30),  # Good flow rate
            "heating_cycles": np.ones(30) * 12
            + np.random.normal(0, 0.5, 30),  # Fast heating
        ...
        high_severity_actions = [
            ...
            if a.severity in (ActionSeverity.HIGH, ActionSeverity.CRITICAL)
        ]
        assert len(high_severity_actions) == 0
