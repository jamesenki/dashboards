...
class TestDescalingRequirementPrediction:
    ...
    def recently_descaled_features(self):
        ...
        return {
            ...
            "maintenance_history": [
                ...
                {"type": "descaling", "date": datetime.now() - timedelta(days=20)},  # Recent descaling
    ...
    @pytest.mark.asyncio
    async def test_descaling_prediction_needs_descaling(self, descaling_model, sample_features):
    ...
    @pytest.mark.asyncio
    async def test_descaling_prediction_recently_descaled(self, descaling_model, recently_descaled_features):
        """Test prediction with features indicating recent descaling."""
        # Perform prediction
        result = await descaling_model.predict(
            recently_descaled_features["device_id"], 
            recently_descaled_features
        )
    ...
    async def test_action_recommendations(self, descaling_recommender):
        ...
        high_severity_actions = [a for a in actions if a.severity in [ActionSeverity.HIGH, ActionSeverity.MEDIUM]]
