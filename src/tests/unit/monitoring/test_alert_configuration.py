...
class TestAlertConfiguration(unittest.TestCase):
    ...
    def test_create_alert_rule(self):
        ...
        response = self.client.post(
            f"/models/{model_id}/versions/{version}/alerts/rules", json=new_rule
        )
        ...
        self.assertEqual(
            response.json(),
            {"id": "new-rule-123", "status": "success", "is_mock_data": False},
        )
    ...
    def test_delete_alert_rule(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(
            response.json(), {"status": "success", "message": "Alert rule deleted"}
        )
