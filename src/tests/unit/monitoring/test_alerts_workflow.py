...
async def test_create_alert_rule(sql_repo):
    ...
    try:
        ...
        result_cursor = await sql_repo.db.execute(
...
def test_api_get_alerts():
    ...
    assert (
        found_test_alert
    ), f"Test alert {test_alert_id} not found in response. Alerts count: {len(alerts)}"

    print(
        f"Successfully retrieved alerts from API. Found {len(alerts)} alerts including our test alert."
    )
