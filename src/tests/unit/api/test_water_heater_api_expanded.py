...
class TestWaterHeaterAPIExpanded:
    ...
    def test_update_mode(self, mock_service, client, sample_water_heater):
        ...
        response = client.patch("/api/water-heaters/test-water-heater-1/mode", json=payload)
    ...
    @patch('src.api.water_heater.router.service')
    def test_add_temperature_reading(self, mock_service, client, sample_water_heater, sample_reading):
        """Test adding a temperature reading to a water heater."""
        # Mock the service methods
        mock_service.get_water_heater = AsyncMock(return_value=sample_water_heater)
        updated_heater = sample_water_heater.model_copy(update={
        ...
        response = client.post("/api/water-heaters/test-water-heater-1/readings", json=payload)
    ...
    @patch('src.api.water_heater.router.service')
    def test_get_water_heater_with_readings(self, mock_service, client, sample_water_heater, sample_reading):
        ...
        mock_service.get_water_heater = AsyncMock(return_value=water_heater_with_readings)
    ...
    @patch('src.api.water_heater.router.service')
    def test_invalid_temperature_update(self, mock_service, client, sample_water_heater):
        """Test updating a water heater with an invalid temperature."""
        # Mock the service to raise an error
        mock_service.get_water_heater = AsyncMock(return_value=sample_water_heater)
        mock_service.update_target_temperature = AsyncMock(side_effect=ValueError("Temperature out of range"))
