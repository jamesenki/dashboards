...
from src.models.vending_machine import (
    ...
    VendingMachineStatus, VendingMachineMode, LocationType, UseType, SubLocation
)
from src.models.device import DeviceStatus, DeviceType

# Import the data generator module (now implemented)
from src.data_generators.vending_machine_data_generator import VendingMachineDataGenerator
...
class TestVendingMachineDataGenerator:
    ...
    def test_generate_single_vending_machine(self):
        ...
        assert vending_machine.location_type in list(LocationType) or vending_machine.location_type is None
        assert vending_machine.use_type in list(UseType) or vending_machine.use_type is None
    ...
    def test_generate_vending_machine_readings(self):
        ...
        for reading in readings:
            assert reading.temperature is None or (1 <= reading.temperature <= 10)  # Celsius for refrigerated
            assert reading.power_consumption is None or (0 <= reading.power_consumption <= 500)  # watts
    ...
    def test_generate_vending_machine_with_usage_pattern(self):
        ...
        vending_machine = generator.generate_vending_machine(id="test-vm-high-traffic", usage_pattern="high_traffic")
        readings = generator.generate_vending_machine_readings(
            vending_machine=vending_machine,
            start_time=datetime.now() - timedelta(days=1),
            end_time=datetime.now(),
        ...
        vending_machine = generator.generate_vending_machine(id="test-vm-business", usage_pattern="business_hours")
        readings = generator.generate_vending_machine_readings(
            vending_machine=vending_machine,
            start_time=datetime.now() - timedelta(days=1),
            end_time=datetime.now(),
        ...
        non_business_readings = [r for r in readings if r.timestamp.hour < 9 or r.timestamp.hour >= 17]
    ...
    def test_generate_vending_machine_with_stock_levels(self):
        ...
        products = generator.generate_products(count=10)
        generator.add_products_to_vending_machine(full_machine, products, stock_level="full")
        ...
        products = generator.generate_products(count=10)
        generator.add_products_to_vending_machine(empty_machine, products, stock_level="low")
