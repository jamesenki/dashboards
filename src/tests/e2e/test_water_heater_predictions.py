...
class TestWaterHeaterPredictions:
    ...
    def test_prediction_api_endpoints(self):
        ...
        lifespan_response = client.get(f"/api/predictions/water-heaters/{self.heater_id}/lifespan-estimation")
        assert lifespan_response.status_code == 200, f"Lifespan prediction failed with status {lifespan_response.status_code}"
        ...
        anomaly_response = client.get(f"/api/predictions/water-heaters/{self.heater_id}/anomaly-detection")
        assert anomaly_response.status_code == 200, f"Anomaly detection failed with status {anomaly_response.status_code}"
        ...
        usage_response = client.get(f"/api/predictions/water-heaters/{self.heater_id}/usage-patterns")
        assert usage_response.status_code == 200, f"Usage patterns failed with status {usage_response.status_code}"
        ...
        multi_response = client.get(f"/api/predictions/water-heaters/{self.heater_id}/multi-factor")
        assert multi_response.status_code == 200, f"Multi-factor prediction failed with status {multi_response.status_code}"
        ...
        all_response = client.get(f"/api/predictions/water-heaters/{self.heater_id}/all")
        assert all_response.status_code == 200, f"All predictions failed with status {all_response.status_code}"
        all_data = all_response.json()
        assert len(all_data) >= 4, f"Expected at least 4 predictions, got {len(all_data)}"
    ...
    def test_prediction_data_structure(self):
        ...
        assert "estimated_remaining_years" in raw_details or "estimated_remaining_days" in raw_details
    ...
    def test_prediction_ui_integration(self):
        ...
        prediction_section = soup.select_one("#prediction-section") or soup.select_one(".prediction-section")
        assert prediction_section, "Prediction section not found in the UI"
        
        # Check for specific prediction widgets or components
        lifespan_widget = soup.select_one("#lifespan-prediction") or soup.select_one(".lifespan-prediction")
        assert lifespan_widget, "Lifespan prediction widget not found"
        
        anomaly_widget = soup.select_one("#anomaly-prediction") or soup.select_one(".anomaly-prediction")
        assert anomaly_widget, "Anomaly prediction widget not found"
        
        # Find any prediction loading indicators
        loading_indicators = soup.select(".prediction-loading") or soup.select(".loading-indicator")
        
        # Check for script that initializes predictions
        prediction_scripts = []
        for script in soup.find_all('script'):
            if script.string and ("prediction" in script.string.lower() or "forecast" in script.string.lower()):
