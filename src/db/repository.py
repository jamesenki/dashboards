...
class DeviceRepository:
            ...
            if self.fallback_enabled and self.cache.client:
                try:
                    ...
                    if cached_devices:
                        ...
                        logger.info(
                            f"Retrieved {len(devices)} devices from cache fallback"
                        )
                        return devices
    ...
    async def get_device(self, device_id: str) -> Optional[Device]:
        ...
        if self.cache.client:
            try:
                ...
                logger.warning(
                    f"Cache retrieval error for device {device_id}: {str(cache_error)}"
                )
        ...
        try:
            ...
            if device and self.cache.client:
                try:
                    ...
                    logger.warning(
                        f"Failed to update cache for device {device_id}: {str(cache_set_error)}"
                    )
            ...
            logger.error(
                f"Database error in get_device for {device_id}: {str(db_error)}"
            )
    ...
    async def create_device(self, device: Device) -> Device:
        ...
        if self.cache.client:
            ...
            await self.cache.publish(
                "device_events", {"event": "created", "device_id": device.id}
            )
    ...
    async def update_device(
        ...
        if updated_device and self.cache.client:
            ...
            await self.cache.publish(
                "device_events", {"event": "updated", "device_id": device_id}
            )
    ...
    async def delete_device(self, device_id: str) -> bool:
        ...
        if success and self.cache.client:
            ...
            await self.cache.publish(
                "device_events", {"event": "deleted", "device_id": device_id}
            )
    ...
    async def add_device_reading(
        ...
        if device and self.cache.client:
            # Cache the reading
            await self.cache.cache_latest_reading(device_id, reading.dict())

            # Update device state
            await self.cache.set_device_state(device_id, device.dict())

            # Publish reading event for real-time updates
            await self.cache.publish(f"device:{device_id}:readings", reading.dict())

            # Also publish to the general readings channel
            await self.cache.publish(
                "device_readings", {"device_id": device_id, "reading": reading.dict()}
            )
    ...
    async def get_device_readings(
        ...
        if offset == 0 and not start_time and not end_time and self.cache.client:
            if metric_name:
                ...
                cached_reading = await self.cache.get(
                    f"device:{device_id}:latest:{metric_name}"
                )
                if cached_reading:
                ...
                if latest_readings:
                    return [
                        DeviceReading.parse_obj(reading)
