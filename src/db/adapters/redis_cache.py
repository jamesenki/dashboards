...
class RedisCache:
    ...
    async def connect(self) -> None:
        ...
        if (self.last_connection_attempt and 
            (current_time - self.last_connection_attempt).total_seconds() < 30 and
        ...
        try:
            ...
            if self.connection_attempts >= self.max_connection_attempts:
                logger.warning("Max Redis connection attempts reached. Will use fallback mechanisms.")
    ...
    async def set_device_state(self, device_id: str, state: Dict) -> bool:
        ...
        return await self.set(f"device:{device_id}", state, expire=self.device_state_ttl)
...
async def get_redis_cache() -> RedisCache:
    ...
    if redis_cache.client is None:
        ...
        try:
            ...
            if last_attempt and (datetime.now(UTC) - last_attempt).total_seconds() > 300:
