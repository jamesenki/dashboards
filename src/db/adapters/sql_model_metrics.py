...
class SQLModelMetricsRepository:
    ...
    async def get_models(self) -> List[Dict[str, Any]]:
        ...
        for model in models_data:
            ...
            metrics = {row['metric_name']: row['metric_value'] for row in metrics_result}
            
            # Add health status based on metrics
            if metrics.get('accuracy', 0) > 0.9 and metrics.get('drift_score', 1) < 0.05:
                health_status = 'GREEN'
            elif metrics.get('accuracy', 0) > 0.8 and metrics.get('drift_score', 1) < 0.1:
