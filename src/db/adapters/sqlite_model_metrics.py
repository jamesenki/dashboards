...
class SQLiteModelMetricsRepository:
    ...
    def _ensure_model_exists(self, model_id: str, model_version: str) -> bool:
        ...
        try:
            ...
            if not result:
                ...
                self.db.execute(
                    ...
                    (
                        model_id,
                        f"Model {model_id}",
                        f"Auto-created model for version {model_version}",
                    ),
                )
                self.logger.info(
                    f"Created missing model '{model_id}' to satisfy foreign key constraint"
                )
            ...
            if not version_result:
                ...
                self.logger.info(
                    f"Created missing model version '{model_version}' for model '{model_id}'"
                )
    ...
    async def record_model_metrics(
        ...
        try:
            ...
            self.logger.info(
                f"Successfully recorded {len(values)} metrics for model {model_id} version {model_version}"
            )
        except Exception as e:
    ...
    async def get_model_metrics_history(
        ...
        for record in results:
            history.append(
                {"timestamp": record["timestamp"], "value": record["metric_value"]}
            )
        ...
        try:
            ...
            for rule in results:
                ...
                rule_dict = {
                    ...
                    "rule_name": rule.get(
                        "rule_name", f"Alert for {rule['metric_name']}"
                    ),
                    "description": rule.get(
                        "description", f"Alert for {rule['metric_name']}"
                    ),
                }
                ...
                if operator_value:
                    ...
                    rule_dict[
                        "condition"
                    ] = operator_value  # Include both for compatibility
                else:
                    # Default operators based on metric type conventions
                    if rule["metric_name"] in [
                        "accuracy",
                        "precision",
                        "recall",
                        "f1_score",
                        ...
                        rule_dict[
                            "operator"
                        ] = "<"  # For accuracy metrics, alert when below threshold
                        rule_dict["condition"] = "<"
                    else:
                        rule_dict[
                            "operator"
                        ] = ">"  # For error metrics, alert when above threshold
                        rule_dict["condition"] = ">"
    ...
    async def get_archived_models(self) -> List[Dict[str, Any]]:
        ...
        for model in models_data:
            ...
            WHERE
                m1.model_id = ?
            """
            latest_metrics_data = self.db.fetch_all(
    ...
    async def get_triggered_alerts(
        ...
        try:
            ...
            rule_column_names = (
                [col["name"] for col in rule_columns] if rule_columns else []
            )
            ...
            event_column_names = (
                [col["name"] for col in event_columns] if event_columns else []
            )
