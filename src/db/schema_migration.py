...
def apply_migrations(connection):
    ...
    if current_version < 2:
        logger.info("Applying migration to version 2: Adding model_version to alert_rules table")
        try:
            ...
            if cursor.fetchone() is not None:
                ...
                try:
                    cursor.execute("ALTER TABLE alert_rules ADD COLUMN model_version TEXT")
                    logger.info("Added model_version column to alert_rules table")
                except sqlite3.OperationalError as e:
                    if "duplicate column name" in str(e):
                        logger.info("model_version column already exists in alert_rules table")
                    else:
                        raise
    ...
    if current_version < 3:
        logger.info("Applying migration to version 3: Adding rule_name to alert_rules table")
        try:
            # Check if the alert_rules table exists
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='alert_rules'")
            if cursor.fetchone() is not None:
                ...
                try:
                    ...
                    if "duplicate column name" in str(e):
                        logger.info("rule_name column already exists in alert_rules table")
                    else:
                        raise
    ...
    if current_version < 4:
        logger.info("Applying migration to version 4: Adding operator to alert_rules table")
        try:
            # Check if the alert_rules table exists
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='alert_rules'")
            if cursor.fetchone() is not None:
    ...
    if current_version < 5:
        logger.info("Applying migration to version 5: Adding description to alert_rules table")
        try:
            # Check if the alert_rules table exists
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='alert_rules'")
            if cursor.fetchone() is not None:
                ...
                if "description" not in column_names:
                    ...
                    cursor.execute("ALTER TABLE alert_rules ADD COLUMN description TEXT")
                    logger.info("Added description column to alert_rules table")
                else:
                    logger.info("description column already exists in alert_rules table")
    ...
    if current_version < 6:
        logger.info("Applying migration to version 6: Adding is_active to alert_rules table")
        try:
            # Check if the alert_rules table exists
            cursor.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='alert_rules'")
            if cursor.fetchone() is not None:
...
# Migration for version 7: Reset and recreate the alert_rules table with proper schema
    if current_version < 7:
        logger.info("Applying migration to version 7: Resetting alert_rules table with proper schema")
        try:
            ...
            if cursor.fetchone() is not None:
                ...
                try:
                    ...
                    if 'severity' not in column_names:
                        ...
                        cursor.execute("ALTER TABLE alert_rules ADD COLUMN severity TEXT DEFAULT 'WARNING'")
                        logger.info("Added severity column to alert_rules table before backup")
                    
                    # Create a backup table with a timestamp to avoid conflicts
                    import time
                    timestamp = int(time.time())
                    backup_table_name = f"alert_rules_backup_{timestamp}"
                    cursor.execute(f"CREATE TABLE {backup_table_name} AS SELECT * FROM alert_rules")
            ...
            if cursor.fetchone() is not None:
                try:
                    ...
                    # Find common columns
                    common_columns = [col for col in backup_column_names if col in new_column_names]
    ...
    logger.info(f"Applied {migrations_applied} migrations. Schema version now at {CURRENT_SCHEMA_VERSION}.")
