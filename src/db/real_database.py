...
class SQLiteDatabase:
        ...
        try:
            ...
            for model in models_data:
                ...
                WHERE
                    m1.model_id = ?
                """
                latest_metrics_data = self.fetch_all(
                    latest_metrics_query, (model_id, model_id)
                )
    ...
    def _ensure_legacy_tables_exist(self):
        ...
        if cursor.fetchone() is None:
            logger.info(
                "Creating legacy alert_rules_old table to support existing code references"
            )
    ...
    def populate_test_data(self):
        ...
        models = [
            (
                ...
                "Predictive model for water heater efficiency",
                False,
            ),
            (
                "anomaly-detection-1",
            ...
            (
                ...
                "Forecasts energy usage patterns",
                False,
            ),
            (
                "maintenance-predictor-1",
                "Maintenance Need Predictor",
        ...
        versions = [
            (
                ...
                "water-heater-model-1",
                "1.0",
                "/models/water-heater/v1.pkl",
            ),
            (
                "water-heater-v2",
                "water-heater-model-1",
                "1.1",
                "/models/water-heater/v2.pkl",
            ),
            (
                "water-heater-v3",
                "water-heater-model-1",
                "1.2",
                "/models/water-heater/v3.pkl",
            ),
        ...
        for model_id, versions in {
            ...
            for version in versions:
                ...
                for days_ago in range(30, 0, -1):
                    ...
                    drift_trend = (
                        days_ago / 1000 if model_id == "water-heater-model-1" else 0
                    )
                    metrics.append(
                        (
                            str(uuid.uuid4()),
        ...
        rules = [
            ...
            (
                ...
                "<",
                "WARNING",
                "Alert when precision falls below threshold",
                datetime.now().isoformat(),
                1,
            ),
            (
        ...
        models = [
            (
                "water-heater-model-1",
                "Water Heater Predictive Model",
                "Model for predicting water heater issues",
            ),
            (
                "anomaly-detection-1",
                "Anomaly Detection Model",
                "Detects anomalies in water heater operation",
            ),
            (
                "energy-forecasting-1",
        ...
        try:
            ...
            if existing_rules:
                ...
                for i, rule in enumerate(
                    existing_rules[:2]
                ):  # Use first two rules for events
                    ...
                    created_at = (
                        datetime.now() - timedelta(days=2 if i == 0 else 5)
                    ).isoformat()
                    resolved_at = (
                        None
                        if i == 0
                        else (datetime.now() - timedelta(days=4)).isoformat()
                    )
        ...
        self.execute_batch(
            "INSERT OR REPLACE INTO model_tags (id, name, color) VALUES (?, ?, ?)", tags
        )
