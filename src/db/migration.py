...
from src.db.models import Base, DeviceModel, DiagnosticCodeModel, ReadingModel
...
async def create_tables() -> None:
    ...
    async with engine.begin() as conn:
        ...
        for table in [
            ...
            ReadingModel.__table__,
            DiagnosticCodeModel.__table__,
        ]:
            try:
            ...
            try:
                ...
                if timescale_exists:
                    ...
                    try:
                        ...
                        logger.info(
                            "Configured readings table as TimescaleDB hypertable"
                        )
                    except Exception as e:
                        logger.warning(
                            f"Cannot convert readings table to TimescaleDB hypertable: {e}"
                        )
                        logger.info(
                            "Continuing with standard PostgreSQL table instead of TimescaleDB hypertable"
                        )
                else:
                    logger.info(
                        "TimescaleDB extension is not installed - using standard PostgreSQL tables"
                    )
                    logger.info(
                        "For time-series performance enhancements, consider installing TimescaleDB:"
                    )
                    logger.info(
                        "  1. Uninstall current timescaledb: brew uninstall timescaledb"
                    )
                    logger.info(
                        "  2. Install for PostgreSQL 14: brew install timescaledb --postgresql@14"
                    )
                    logger.info(
                        "  3. Run timescaledb-tune: /opt/homebrew/bin/timescaledb-tune --quiet --yes"
                    )
                    logger.info(
                        "  4. Restart PostgreSQL: brew services restart postgresql@14"
                    )
                    logger.info(
                        '  5. Create extension: psql -U iotsphere iotsphere -c "CREATE EXTENSION IF NOT EXISTS timescaledb CASCADE;"'
                    )
            except Exception as e:
                logger.info(
                    "Cannot verify TimescaleDB status - continuing with standard PostgreSQL"
                )
