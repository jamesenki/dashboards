...
class SalesData(BaseModel):
    ...
    product_sales: List[ProductSale] = Field(
...
class UsagePattern(BaseModel):
    ...
    time_of_day: Dict[TimeOfDay, int] = Field(
        default_factory=dict, description="Sales by time of day"
    )
    day_of_week: Dict[DayOfWeek, int] = Field(
        default_factory=dict, description="Sales by day of week"
    )
    popular_products: List[ProductSale] = Field(
        default_factory=list, description="Most popular products"
    )
...
class MaintenanceEvent(BaseModel):
    ...
    technician: Optional[str] = Field(
        None, description="Person who performed maintenance"
    )
    description: Optional[str] = Field(
        None, description="Description of maintenance performed"
    )
    parts_replaced: Optional[List[str]] = Field(
        None, description="Parts that were replaced"
    )
    cost: Optional[float] = Field(None, description="Cost of maintenance")
    duration_minutes: Optional[int] = Field(
        None, description="Duration of maintenance in minutes"
    )
...
class MaintenanceHistory(BaseModel):
    ...
    events: List[MaintenanceEvent] = Field(
        default_factory=list, description="Maintenance events"
    )
    last_maintenance: Optional[datetime] = Field(
        None, description="Last maintenance date"
    )
...
class RefillHistory(BaseModel):
    ...
    most_refilled_product: Optional[str] = Field(
        None, description="Most frequently refilled product"
    )
...
class TemperatureReading(BaseModel):
    ...
    is_normal: bool = Field(
        True, description="Whether temperature is within normal range"
    )
...
class TemperatureTrends(BaseModel):
    ...
    readings: List[TemperatureReading] = Field(
        default_factory=list, description="Temperature readings"
    )
    current_temperature: Optional[float] = Field(
        None, description="Current temperature (most recent reading)"
    )
    average_temperature: Optional[float] = Field(
        None, description="Average temperature"
    )
...
class MachineStatus(BaseModel):
    ...
    machine: str = Field(
        "Online", description="Overall machine status (Online/Offline)"
    )
    ...
    customer_door: str = Field(
        "Closed", description="Customer door status (Open/Closed)"
    )
...
class GaugeData(BaseModel):
    ...
    needle_value: float = Field(
        ..., description="Percentage value for needle position (0-100)"
    )
    display_suffix: Optional[str] = Field(
        None, description="Unit suffix for display (e.g., '°F', 'lb', 'sec')"
    )
    status: Optional[str] = Field(
        None, description="Status indicator (e.g., 'Normal', 'Warning')"
    )
...
class GaugeIndicators(BaseModel):
    ...
    freezer_temperature: GaugeData = Field(
...
class AssetHealth(BaseModel):
    ...
    asset_id: str = Field(
        ..., description="Unique identifier for the ice cream machine"
    )
    ...
    freezer_temperature: Dict[str, Any] = Field(
        ..., description="Freezer temperature gauge data"
    )
    ...
    # Inventory data
    inventory: List[IceCreamInventoryItem] = Field(
        default_factory=list, description="Ice cream flavor inventory"
    )
...
class LocationPerformance(BaseModel):
    ...
    asset_count: int = Field(
        ..., description="Number of ice cream machines at this location"
    )
    online_count: int = Field(
        ..., description="Number of online machines at this location"
    )
...
class OperationsSummary(BaseModel):
    ...
    total_assets_online: int = Field(
        ..., description="Number of online ice cream machines"
    )
    service_tickets_open: int = Field(..., description="Number of open service tickets")
    service_tickets_closed: int = Field(
        ..., description="Number of closed service tickets"
    )
    ...
    average_sales_per_asset: float = Field(
        ..., description="Average sales per ice cream machine"
    )

    # Performance by location
    locations: List[LocationPerformance] = Field(
        default_factory=list, description="Performance metrics by location"
    )

    # Open service tickets
    open_tickets: List[ServiceTicket] = Field(
        default_factory=list, description="List of open service tickets"
    )

    # Sales data by ice cream flavor
    sales_by_flavor: Dict[str, float] = Field(
        default_factory=dict, description="Sales amount by ice cream flavor"
    )

    # Summary statistics
    stats: Dict[str, Any] = Field(
        default_factory=dict, description="Additional summary statistics"
    )
...
class OperationalStatus(BaseModel):
    ...
    freezer_temperature: Dict[str, Any] = Field(
        ..., description="Freezer temperature data"
    )
    max_ram_load: Dict[str, Any] = Field(..., description="Maximum RAM load data")
    cycle_time: Dict[str, Any] = Field(..., description="Cycle time data")

    # Inventory tracking
    ice_cream_inventory: List[IceCreamInventoryItem] = Field(
        default_factory=list, description="Ice cream flavor inventory"
    )
