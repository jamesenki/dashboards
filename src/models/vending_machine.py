...
class ProductItem(BaseModel):
    ...
    location: Optional[str] = Field(
        None, description="Location in vending machine (slot/row)"
    )
...
class VendingMachineReading(BaseModel):
    ...
    temperature: Optional[float] = Field(
        None, description="Internal temperature in Celsius"
    )
    power_consumption: Optional[float] = Field(
        None, description="Power consumption in watts"
    )
    ...
    sales_count: Optional[int] = Field(
        None, description="Number of sales in this reading period"
    )
...
class VendingMachine(Device):
    ...
    type: Literal[DeviceType.VENDING_MACHINE] = Field(
        DeviceType.VENDING_MACHINE, description="Device type"
    )

    # Machine specifications
    model_number: Optional[str] = Field(
        None, description="Vending machine model number"
    )
    serial_number: Optional[str] = Field(
        None, description="Vending machine serial number"
    )
    machine_status: VendingMachineStatus = Field(
        VendingMachineStatus.OPERATIONAL, description="Current operational status"
    )
    mode: VendingMachineMode = Field(
        VendingMachineMode.NORMAL, description="Current operational mode"
    )

    # Operational settings
    temperature: Optional[float] = Field(
        None, description="Current internal temperature in Celsius"
    )
    ...
    location_business_name: Optional[str] = Field(
        None, description="Business name where machine is located"
    )
    location_type: Optional[LocationType] = Field(
        None, description="Type of location (fast food, hospital, etc.)"
    )
    sub_location: Optional[SubLocation] = Field(
        None, description="Specific area within the location (hallway, lobby, etc.)"
    )
    use_type: Optional[UseType] = Field(
        None, description="Type of usage (serviced, self-serve, etc.)"
    )

    # Data generation properties
    usage_pattern: Optional[str] = Field(
        None,
        description="Usage pattern for data generation (e.g., high_traffic, business_hours)",
    )
    stock_level: Optional[str] = Field(
        None, description="Stock level for data generation (e.g., full, low)"
    )

    # Maintenance information
    maintenance_partner: Optional[str] = Field(
        None, description="Service provider for maintenance"
    )
    ...
    next_maintenance_date: Optional[datetime] = Field(
        None, description="Scheduled date for next maintenance"
    )

    # Inventory
    products: List[ProductItem] = Field(
        default_factory=list, description="Products currently in stock"
    )

    # Sensor readings history
    readings: List[VendingMachineReading] = Field(
