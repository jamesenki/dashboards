...
class DeviceReading(BaseModel):
    ...
    timestamp: datetime = Field(
        default_factory=datetime.utcnow, description="Time when reading was taken"
    )
    metric_name: Optional[str] = Field(
        None, description="Name of the metric (if single-metric reading)"
    )
    value: Optional[Any] = Field(
        None, description="Value of the metric (if single-metric reading)"
    )
