...
class WaterHeaterHealthDashboard:
    ...
    def _render_lifespan_overview(self) -> str:
        ...
        if self.lifespan_prediction.raw_details:
            if "total_expected_lifespan_years" in self.lifespan_prediction.raw_details:
                remaining_years = remaining_percentage * self.lifespan_prediction.raw_details["total_expected_lifespan_years"]
            
            if "estimated_end_of_life_date" in self.lifespan_prediction.raw_details:
                try:
                    end_of_life_date = datetime.fromisoformat(self.lifespan_prediction.raw_details["estimated_end_of_life_date"])
                except (ValueError, TypeError):
                    # If date parsing fails, calculate based on remaining percentage
                    end_of_life_date = datetime.now() + timedelta(days=int(remaining_years * 365))
        ...
        lifespan_factors = []
        if self.lifespan_prediction.raw_details and "factors_affecting_lifespan" in self.lifespan_prediction.raw_details:
            lifespan_factors = self.lifespan_prediction.raw_details["factors_affecting_lifespan"]
    ...
    def _render_component_health_overview(self) -> str:
        ...
        component = ComponentFailurePredictionComponent(self.component_failure_prediction)
    ...
    def _render_time_to_failure_predictions(self) -> str:
        ...
        sorted_components = sorted(
            self.time_to_failure.items(),
            key=lambda x: x[1]["expected_failure_date"]
        )
        ...
        for component, failure_data in sorted_components:
            label = self.component_labels.get(component, component.replace("_", " ").title())
    ...
    def _render_combined_recommendations(self) -> str:
        ...
        for i, action in enumerate(self.combined_recommendations):
            ...
            due_date = action.due_date.strftime("%Y-%m-%d") if action.due_date else "No date specified"
    ...
    def _calculate_time_to_failure(self) -> Dict[str, Dict[str, Any]]:
        ...
        """
        if not self.component_failure_prediction or not self.component_failure_prediction.raw_details:
            return {}
        ...
        total_expected_lifespan_years = lifespan_details.get("total_expected_lifespan_years", 10)
        ...
        for component, failure_probability in components.items():
            ...
            if component == "anode_rod":
                component_max_years = min(total_expected_lifespan_years, 3)  # Typically 3-5 years
            elif component == "heating_element":
                component_max_years = min(total_expected_lifespan_years, 5)  # Typically 4-6 years
    ...
    def _combine_recommendations(self) -> List[RecommendedAction]:
        ...
        if self.component_failure_prediction and self.component_failure_prediction.recommended_actions:
        ...
        def priority_key(action):
            ...
            date_priority = (action.due_date.timestamp() if action.due_date else float('inf'))
