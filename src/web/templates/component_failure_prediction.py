...
from src.predictions.interfaces import PredictionResult, ActionSeverity
...
class ComponentFailurePredictionComponent:
    ...
    def render(self) -> str:
        ...
        """
        has_prediction_data = (
            self.prediction.raw_details and 
            "components" in self.prediction.raw_details and 
            len(self.prediction.raw_details["components"]) > 0
        )
        ...
        if has_prediction_data:
            ...
            sorted_components = sorted(
                [(comp, prob) for comp, prob in components.items() 
