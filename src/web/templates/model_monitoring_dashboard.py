...
class ModelMonitoringDashboard:
        ...
        if monitoring_service:
            try:
                ...
                if self.models and len(self.models) > 0:
                    ...
                    self.metrics = monitoring_service.get_latest_metrics(model_id, model_version)
    ...
    def _render_metrics(self) -> str:
        ...
        for metric_name, metric_value in self.metrics.items():
            value_formatted = f"{metric_value:.2f}" if isinstance(metric_value, float) else metric_value
            html += f"""
