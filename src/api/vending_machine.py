...
class VendingMachineCreate(BaseModel):
    ...
    location_type: Optional[LocationType] = Field(
        None, description="Type of location (retail, office, etc.)"
    )
    sub_location: Optional[str] = Field(
        None, description="Specific area within the location"
    )
    use_type: Optional[UseType] = Field(
        None, description="Type of usage (public, employee, etc.)"
    )
...
class VendingMachineUpdate(BaseModel):
    ...
    model_number: Optional[str] = Field(None, description="Model number")
    serial_number: Optional[str] = Field(None, description="Serial number")
    machine_status: Optional[VendingMachineStatus] = Field(
        None, description="Current operational status"
    )
    mode: Optional[VendingMachineMode] = Field(
        None, description="Current operational mode"
    )
    temperature: Optional[float] = Field(
        None, description="Internal temperature in Celsius"
    )
...
async def get_all_vending_machines(
    ...
    format_names: bool = Query(
        False, description="Format machine names with pipe-delimited location info"
    ),
):
        ...
        enhanced_machines.append(
            {
                ...
                "location_type": str(machine.location_type.value)
                if machine.location_type
                else None,
                "sub_location": str(machine.sub_location.value)
                if machine.sub_location
                else None,
                "location": machine.location,
                "status": str(machine.machine_status.value)
                if machine.machine_status
                else "UNKNOWN",
            }
        )
...
async def delete_vending_machine(
    ...
    if not result:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="Vending machine not found"
        )
    return None
...
async def update_product_quantity(
    ...
    try:
        return service.update_product_quantity(
            vm_id, product_id, quantity_update.quantity_change if quantity_update else 0
        )
    except ValueError as e:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(e))
