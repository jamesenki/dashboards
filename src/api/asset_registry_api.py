...
async def get_device_metadata(request) -> web.Response:
    ...
    try:
        ...
        return web.json_response(
            {"error": f"Failed to retrieve device metadata: {str(e)}"},
            status=500
        )
...
async def update_device_metadata(request) -> web.Response:
    ...
    try:
        ...
        updated_metadata = await asset_service.update_device_metadata(device_id, updates)
        ...
        return web.json_response(
            {"error": f"Failed to update device metadata: {str(e)}"},
            status=500
        )
