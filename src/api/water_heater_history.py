...
async def get_history_dashboard(
    ...
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Water heater with ID {heater_id} not found")

@router.get("/water-heaters/{heater_id}/history/temperature", response_model=Dict[str, Any])
...
@router.get("/water-heaters/{heater_id}/history/pressure-flow", response_model=Dict[str, Any])
