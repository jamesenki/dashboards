...
from src.services.vending_machine_operations_service import VendingMachineOperationsService
...
def get_fleet_operations_summary(
    service: VendingMachineOperationsService = Depends(get_vending_machine_operations_service)
):
...
def get_asset_health(
    ...
    try:
        ...
        freezer_ok = float(operational_status.freezer_temperature.get("freezerTemperature", 0)) < 0
        ...
        return AssetHealth(
            ...
            asset_health={
                "value": "85%",
                "status": "OK" if (freezer_ok and ram_ok and dispense_ok) else "Warning"
            },
...
def get_sales_data(
    machine_id: str = Path(..., description="Ice cream machine ID"),
    period: str = Query("week", description="Sales data period (day, week, month, quarter, year)"),
    service: VendingMachineOperationsService = Depends(get_vending_machine_operations_service)
):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST if "period" in str(e).lower() 
