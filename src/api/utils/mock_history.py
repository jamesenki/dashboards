...
# Import our datetime handler for consistent date handling
from src.api.utils.datetime_handler import format_date_for_api, generate_date_range
        ...
        "anomalies": [
            {
                ...
                "timestamp": (
                    datetime.now() - timedelta(days=random.randint(1, days - 1))
                ).isoformat(),
...
def create_mock_pressure_flow_history(device_id: str, days: int) -> Dict[str, Any]:
    ...
    for i in range(days):
        ...
        daily_pressure = (
            base_pressure + random.uniform(-3, 3) + math.sin(i / 5 * math.pi) * 1.5
        )
