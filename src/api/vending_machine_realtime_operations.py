...
# Create router
router = APIRouter(prefix="/vending-machines-realtime", tags=["vending-machine-operations"])
...
def get_realtime_operations_data(
    ...
    service: VendingMachineRealtimeOperationsService = Depends(get_vending_machine_realtime_operations_service)
):
    ...
    try:
        ...
        import logging
        logging.error(f"Error generating realtime operations data for {machine_id}: {str(e)}")
