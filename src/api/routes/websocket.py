...
def create_default_state_for_device_type(
...
async def device_state(websocket: WebSocket, device_id: str):
    ...
    try:
        ...
        try:
            ...
            response = {"type": "state_update", "device_id": device_id, **current_state}
            ...
            logger.info(
                f"No shadow exists for device {device_id}, sending default state"
            )
        ...
        while True:
            ...
            permission_error = await verify_websocket_operation(
                websocket, message_type, message
            )
            if permission_error:
                logger.warning(
                    f"Permission denied for {message_type} operation on device {device_id}"
                )
            ...
            if message_type == "get_state":
                ...
                try:
                    current_state = await shadow_service.get_device_shadow(device_id)
                    # Add the type field needed by the frontend and tests
                    response = {"type": "state_update", **current_state}
                    await websocket.send_text(json.dumps(response))
                except ValueError as e:
                ...
                try:
                    ...
                    await websocket.send_text(
                        json.dumps({"type": "update_error", "error": str(e)})
                    )
                ...
                try:
                    ...
                    await websocket.send_text(
                        json.dumps(
                            {"type": "delta", "device_id": device_id, "delta": delta}
                        )
                    )
                except ValueError as e:
                    await websocket.send_text(
...
async def device_telemetry(websocket: WebSocket, device_id: str):
    ...
    try:
        ...
        if message.get("type") == "subscribe":
                    ...
                    try:
                        ...
                        if message_type == "unsubscribe":
                            ...
                            if not metrics_to_remove:
                                ...
                                websocket.state.subscribed_metrics = [
                                    m
                                    for m in websocket.state.subscribed_metrics
            ...
            await websocket.send_text(
                json.dumps({"type": "error", "error": "Expected subscription request"})
            )
    ...
    try:
        ...
        while True:
            ...
            if message_type == "subscribe":
                ...
                await websocket.send_text(
                    json.dumps(
                        {"type": "unsubscribe_confirmed", "topics": topics_to_unsub}
                    )
                )
