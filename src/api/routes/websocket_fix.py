...
async def broadcast_channel_fixed(websocket: WebSocket):
    ...
    try:
        ...
        while True:
            ...
            logger.info(
                f"Received broadcast message from {user_info['username']}: {message}"
            )
...
async def device_telemetry_fixed(websocket: WebSocket, device_id: str):
    ...
    if not user_info:
        logger.warning(
            f"Unauthorized connection attempt for device {device_id} telemetry"
        )
        await websocket.close(code=status.WS_1008_POLICY_VIOLATION)
        return
            ...
            try:
                ...
                if message_type == "subscribe":
                    ...
                    if not metric:
                        await websocket.send_text(
                            json.dumps(
                                {"type": "error", "error": "Metric name is required"}
                            )
                        )
                        continue
                    ...
                    recent_data = generate_sample_telemetry(
                        device_id, device_type, metric, limit
                    )
...
async def simulate_telemetry(websocket: WebSocket, device_id: str):
    ...
    while True:
        try:
            ...
            for metric in metrics:
                telemetry_data = generate_telemetry_point(
                    device_id, device_type, metric
                )
                await websocket.send_text(
                    json.dumps(
                        {
...
def generate_telemetry_point(device_id: str, device_type: str, metric: str) -> dict:
    ...
    return {"value": value, "timestamp": now, "unit": unit}


def generate_sample_telemetry(
    device_id: str, device_type: str, metric: str, limit: int
) -> list:
    """
