...
async def websocket_debug(websocket: WebSocket):
    ...
    if not (is_test_token or is_debug_mode):
        logger.warning(
            f"Debug endpoint rejected unauthorized connection from {websocket.client}"
        )
    ...
    logger.info(
        f"Debug endpoint authenticated user: {user_info['username']} with role: {user_info['role']}"
    )
