...
async def get_temperature_history(
    ...
    try:
        ...
        logger.error(
            f"Error retrieving temperature history for device {device_id}: {str(e)}"
        )

        # In development mode, return mock data when there's an error
        if os.getenv("IOTSPHERE_ENV", "development") == "development":
            logger.info(
                f"Returning mock temperature history after error for {device_id}"
            )
            return create_mock_temperature_history(device_id, days)
...
async def get_energy_usage_history(
    ...
    try:
        ...
        logger.error(
            f"Error retrieving energy usage history for device {device_id}: {str(e)}"
        )

        # In development mode, return mock data when there's an error
        if os.getenv("IOTSPHERE_ENV", "development") == "development":
            logger.info(
                f"Returning mock energy usage history after error for {device_id}"
            )
            return create_mock_energy_history(device_id, days)
...
async def get_all_history(
    ...
    try:
        ...
        if os.getenv("IOTSPHERE_ENV", "development") == "development":
            logger.info(
                f"Using consistent mock data format for history data for {device_id}"
            )
            ...
            try:
                ...
                temperature = await history_service.get_temperature_history(
                    device_id, days
                )
                energy = await history_service.get_energy_usage_history(device_id, days)
                pressure_flow = await history_service.get_pressure_flow_history(
                    device_id, days
                )
                ...
                if not dashboard or not temperature or not energy or not pressure_flow:
                    logger.info(
                        f"Some history data missing for {device_id}, using mock data"
                    )
                    dashboard = create_mock_history_dashboard(device_id, days)
                    temperature = create_mock_temperature_history(device_id, days)
                    energy = create_mock_energy_history(device_id, days)
        ...
        logger.error(
            f"Error retrieving combined history for device {device_id}: {str(e)}"
        )
        raise HTTPException(
...
async def get_pressure_flow_history(
    ...
    try:
        ...
        logger.error(
            f"Error retrieving pressure flow history for device {device_id}: {str(e)}"
        )

        # In development mode, return mock data when there's an error
        if os.getenv("IOTSPHERE_ENV", "development") == "development":
            logger.info(
                f"Returning mock pressure flow history after error for {device_id}"
            )
            return create_mock_pressure_flow_history(device_id, days)
