...
@router.get(
    "/water-heaters/{device_id}/lifespan-estimation", response_model=PredictionResult
)
...
@router.get(
    "/water-heaters/{device_id}/anomaly-detection", response_model=PredictionResult
)
async def get_anomaly_detection_prediction(
    ...
    """
    prediction = await prediction_service.get_prediction(
        device_id=device_id, prediction_type="anomaly_detection", force_refresh=refresh
    )
...
@router.get(
    "/water-heaters/{device_id}/usage-patterns", response_model=PredictionResult
)
...
async def get_all_predictions(
    ...
    for prediction_type in prediction_types:
        prediction = await prediction_service.get_prediction(
            device_id=device_id, prediction_type=prediction_type, force_refresh=refresh
        )
        if prediction:
