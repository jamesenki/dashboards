...
async def get_water_heater_ids() -> List[str]:
    ...
    async for session in get_db_session():
        ...
        stmt = select(DeviceModel.id).where(
            DeviceModel.type == DeviceType.WATER_HEATER.value
        )
...
async def get_water_heater_telemetry(device_id: str, hours: int = 24) -> Dict[str, Any]:
    ...
    async for session in get_db_session():
        ...
        if not readings:
            ...
            telemetry["pressure"] = (
                np.ones(30) * 2.5 + np.random.normal(0, 0.2, 30)
            ).tolist()
            telemetry["energy_usage"] = (
                np.ones(30) * 1200 + np.random.normal(0, 50, 30)
            ).tolist()
            telemetry["flow_rate"] = np.linspace(10, 9.5, 30).tolist()
            telemetry["heating_cycles"] = (
                np.ones(30) * 15 + np.random.normal(0, 1, 30)
            ).tolist()
            return telemetry
            ...
            if reading.value and isinstance(reading.value, dict):
                ...
                if reading.metric_name == "temperature":
                    telemetry["temperature"].append(
                        float(reading.value.get("value", 65))
                    )
                    ...
                    telemetry["energy_usage"].append(
                        float(reading.value.get("value", 1200))
                    )
                    ...
                    telemetry["heating_cycles"].append(
                        float(reading.value.get("value", 15))
                    )

    logger.info(
        f"Retrieved {len(telemetry['timestamp'])} readings for device {device_id}"
    )

    # If we don't have enough data, add synthetic data
    if len(telemetry["timestamp"]) < 5:
        logger.warning(
            f"Not enough real data for device {device_id}, adding synthetic data"
        )
        ...
        if count > 0:
            ...
            telemetry["pressure"].extend(
                (np.ones(count) * 2.5 + np.random.normal(0, 0.2, count)).tolist()
            )
            telemetry["energy_usage"].extend(
                (np.ones(count) * 1200 + np.random.normal(0, 50, count)).tolist()
            )
            telemetry["flow_rate"].extend(np.linspace(10, 9.5, count).tolist())
            telemetry["heating_cycles"].extend(
                (np.ones(count) * 15 + np.random.normal(0, 1, count)).tolist()
            )
        ...
        if device.properties and "installation_date" in device.properties:
            try:
                ...
                if isinstance(installation_date_value, str):
                    ...
                    isinstance(installation_date_value, dict)
                    and "value" in installation_date_value
                ):
                    # Handle nested JSON structure
                    installation_date = datetime.fromisoformat(
                        installation_date_value["value"]
                    )
                else:
        ...
        if device.properties and "maintenance_history" in device.properties:
            ...
            info["maintenance_history"] = [
                {"type": "regular", "date": last_maintenance},
                {
                    ...
                    "date": last_maintenance - timedelta(days=180),
                },
...
async def main():
    ...
    for idx, device_id in enumerate(water_heater_ids, 1):
        logger.info(
            f"[{idx}/{len(water_heater_ids)}] Processing water heater: {device_id}"
        )
        ...
        logger.info(
            f"  - Overall failure probability: {prediction.predicted_value:.1%}"
        )
        ...
        if prediction.recommended_actions:
            ...
            for action in prediction.recommended_actions:
                logger.info(
                    f"    - {action.description} (Severity: {action.severity.value})"
                )
