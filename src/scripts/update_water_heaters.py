        ...
        try:
            ...
            for device in devices:
                ...
                if capacity is not None:
                    is_commercial = float(capacity) >= 150  # Commercial if 150L or larger
                else:
                ...
                diagnostic_codes = []
                code_list = COMMERCIAL_DIAGNOSTIC_CODES if is_commercial else RESIDENTIAL_DIAGNOSTIC_CODES
                ...
                logger.info(f"Updated water heater {device.id} with type {heater_type} and {len(diagnostic_codes)} diagnostic codes")
    ...
    async for session in session_generator:
        ...
        try:
            ...
            for heater in new_heaters:
                ...
                diagnostic_codes = heater.diagnostic_codes if hasattr(heater, "diagnostic_codes") else []
                ...
                properties = {
                    ...
                    "mode": heater.mode.value if hasattr(heater.mode, "value") else heater.mode,
                    "heater_status": heater.heater_status.value if hasattr(heater.heater_status, "value") else heater.heater_status,
                    "heater_type": heater.heater_type.value if hasattr(heater.heater_type, "value") else heater.heater_type,
                ...
                for dc in diagnostic_codes:
                    db_diagnostic = DiagnosticCodeModel(
                        ...
                        active=dc.active,
                        additional_info=dc.additional_info if hasattr(dc, "additional_info") else None
                    )
                    session.add(db_diagnostic)
                
                logger.info(f"Added new water heater {heater.id} of type {heater.heater_type}")
