        ...
        try:
            ...
            for heater_id, name, properties_str in heaters:
                ...
                if properties and properties.get("heater_type"):
                    logger.info(f"Skipping water heater {heater_id} as it already has a type")
                    continue
                ...
                if capacity is not None and float(capacity) >= 150:
                    is_commercial = True
                    logger.info(f"Classified {heater_id} as commercial based on capacity: {capacity}")
                else:
                    ...
                    for indicator in commercial_indicators:
                        if indicator in name_lower:
                            is_commercial = True
                            logger.info(f"Classified {heater_id} as commercial based on indicator '{indicator}' in name: {name}")
                            break
                ...
                updated_count += 1
                logger.info(f"Updated water heater {heater_id} ({name}) with type {heater_type} using SQL")
