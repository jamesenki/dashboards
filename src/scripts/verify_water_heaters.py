    ...
    async for session in session_generator:
        ...
        try:
            ...
            query = (
                ...
                .where(DeviceModel.type == DeviceType.WATER_HEATER)
            )
            ...
            for heater in heaters:
                ...
                if (
                    ...
                    or heater_type == WaterHeaterType.COMMERCIAL
                ):
                    commercial_count += 1
                    heater_type = "Commercial"
                elif (
                    heater_type == "Residential"
                    or heater_type == WaterHeaterType.RESIDENTIAL
                ):
                    residential_count += 1
                    heater_type = "Residential"
                ...
                diag_query = (
                    select(func.count())
                    .select_from(DiagnosticCodeModel)
                    .where(DiagnosticCodeModel.device_id == heater.id)
                )
                ...
                sample_codes_info = ", ".join(
                    [
                        f"{code.code}: {code.description} ({code.severity})"
                ...
                logger.info(
                    f"  - Specification: {properties.get('specification_link')}"
                )
                logger.info(
                    f"  - Diagnostic Codes: {diag_count} ({sample_codes_info if sample_codes else 'None'})"
                )
