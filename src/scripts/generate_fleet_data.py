...
NUM_READINGS_PER_MACHINE = (
    48  # Number of readings per machine (e.g., 2 days worth of hourly readings)
)
...
def generate_product(product_id):
    ...
    quantity = random.choices(
        [q[0] for q in quantity_weights], weights=[q[1] for q in quantity_weights], k=1
    )[0]
...
def main():
    ...
    for i in range(NUM_MACHINES):
        ...
        print(
            f"  Created {machine.name} ({machine.id}) at {machine.location_business_name}, {machine.sub_location}"
        )
    ...
    print(
        f"Total number of readings generated: {NUM_MACHINES * NUM_READINGS_PER_MACHINE}"
    )
    ...
    try:
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            if proc.info["name"] == "python" and any(
                "src.main" in arg for arg in proc.info["cmdline"] if arg
            ):
                pid = proc.info["pid"]
                ...
                os.system(
                    "cd /Users/lisasimon/repos/IoTSphereAngular/IoTSphere-Refactor && python -m src.main &"
                )
                time.sleep(3)
