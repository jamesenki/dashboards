...
class MaintenancePredictionModel:
    ...
    def train(self, data: pd.DataFrame) -> None:
        ...
        logger.info(
            f"Model trained. Train accuracy: {train_score:.4f}, Test accuracy: {test_score:.4f}"
        )
        self.is_trained = True

    def predict_next_maintenance(
        self, machine_id: str, recent_data: Optional[pd.DataFrame] = None
        ...
        """
        if not self.is_trained and not os.path.exists(
            os.path.join(self.model_dir, "maintenance_model.pkl")
        ):
