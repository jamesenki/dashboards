...
class WaterHeaterSimulator:
    ...
    def __init__(self, device_id, model, manufacturer, initial_state=None, mqtt_client=None):
        """
        ...
        logger.info(f"Initialized water heater simulator: {device_id} ({manufacturer} {model})")
    ...
    def publish_event(self, event_type, severity, message, details=None):
        ...
        try:
            ...
            logger.info(f"Published event for device {self.device_id}: {event_type} - {message}")
            return True
    ...
    def on_message(self, client, userdata, message):
        ...
        try:
            ...
            logger.info(f"Received command for device {self.device_id}: {command.get('command')}")
    ...
    def handle_command(self, command):
        ...
        if cmd == "set_temperature":
            ...
            if setpoint is not None:
                ...
                self.publish_event("temperature_setpoint_changed", "info", 
            ...
            if mode in valid_modes:
                ...
                self.publish_event("mode_changed", "info", f"Operating mode changed to {mode}")
                return True
            else:
                return False
    ...
    def update_simulation(self):
        ...
        if random.random() < 0.0001 and not self.state["error_code"]:  # 0.01% chance per update
...
class WaterHeaterSimulationManager:
    ...
    def create_fleet(self, count, manufacturer=None, model=None):
        ...
        manufacturers = [manufacturer] if manufacturer else [
            "Rheem", "AO Smith", "Bradford White", "State", "Navien"
        ]
        ...
        models = {
            ...
            "Bradford White": ["eF Series", "AeroTherm", "TTW", "Defender", "ElectriFLEX"],
