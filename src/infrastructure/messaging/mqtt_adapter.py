...
class MqttAdapter:
    ...
    def _handle_telemetry(self, device_id, data):
        ...
        event = {
            ...
            "data": {
                k: v for k, v in data.items() if k not in ["device_id", "timestamp"]
            },
    ...
    def _handle_event(self, device_id, data):
        ...
        if "device_id" in data and data["device_id"] != device_id:
            logger.warning(
                f"Device ID mismatch in event: {device_id} vs {data['device_id']}"
            )
        ...
        logger.info(
            f"Forwarded event from device {device_id}: {event['event_type']} - {event['message']}"
        )
    ...
    def _handle_command_response(self, device_id, data):
        ...
        if "device_id" in data and data["device_id"] != device_id:
            logger.warning(
                f"Device ID mismatch in command response: {device_id} vs {data['device_id']}"
            )
        ...
        logger.info(
            f"Forwarded command response from device {device_id}: {event['status']}"
        )
