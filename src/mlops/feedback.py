...
class FeedbackService:
    ...
    def get_feedback_summary(self, model_name: str, days: int = 30) -> Dict[str, Any]:
        """
        ...
        if summary["total_count"] > 0:
            summary["false_positive_rate"] = (
                summary["false_positives"] / summary["total_count"]
            )
            summary["false_negative_rate"] = (
                summary["false_negatives"] / summary["total_count"]
            )
    ...
    def analyze_feedback_patterns(
        self, model_name: str, days: int = 90
    ) -> Dict[str, Any]:
        """
        ...
        feedback = self.get_feedback_for_model(
            model_name=model_name, start_date=datetime.now() - timedelta(days=days)
        )
        ...
        return {
            "error_patterns": sorted(
                common_errors, key=lambda x: x["count"], reverse=True
            ),
            "device_patterns": sorted(
                problem_devices, key=lambda x: x["error_rate"], reverse=True
            ),
        }
