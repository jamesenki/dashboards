    ...
    for wh in water_heaters:
        ...
        if wh.readings:
            for reading in wh.readings:
                if reading.pressure is not None and reading.energy_usage is not None and reading.flow_rate is not None:
...
async def add_readings(water_heaters):
    ...
    for water_heater in water_heaters:
        print(f"Adding readings to water heater: {water_heater.name} (ID: {water_heater.id})")
