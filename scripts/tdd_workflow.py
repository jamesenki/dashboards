...
def run_command(command, cwd=None):
    ...
    try:
        ...
        log_message(f"Command failed with exit code {e.returncode}: {e.stderr}", "ERROR")
        return e.stderr
...
def verify_postgres_running():
    ...
    try:
        ...
        log_message("Try starting PostgreSQL with: brew services start postgresql@14", "INFO")
        return False
...
def check_timescaledb():
    ...
    try:
        ...
        if result:
            ...
            log_message("Application will use standard PostgreSQL tables instead", "INFO")
            return False
...
def check_test_modifications(feature_name):
    ...
    if current_md5 != original_md5:
        ...
        log_message("If you're modifying tests, make sure requirements have changed", "WARNING")
...
def run_with_coverage(feature_name, phase):
    ...
    cmd = f"python -m pytest src/tests -v -m '{marker}' " \
    ...
    if match:
        ...
        if coverage < 90:
            log_message(f"Warning: Coverage is only {coverage}%, aim for at least 90%", "WARNING")
        else:
...
def run_static_analysis(feature_name):
    ...
    snake_case_name = feature_name.replace('-', '_').lower()
    impl_file = PROJECT_ROOT / "src" / "predictions" / "maintenance" / f"{snake_case_name}.py"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Enhanced TDD Workflow Helper for IoTSphere")
    parser.add_argument("phase", choices=["red", "green", "refactor", "cycle", "ci"],
