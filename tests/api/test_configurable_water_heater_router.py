...
class TestConfigurableWaterHeaterRouter:
    ...
    def test_get_water_heaters(
        self, mock_service_class, mock_service, sample_water_heater
    ):
    ...
    def test_update_temperature(
        ...
        # Verify service was called
        mock_service.update_target_temperature.assert_called_once_with(
            sample_water_heater.id, 55.0
        )
    ...
    def test_update_mode(self, mock_service_class, mock_service, sample_water_heater):
        ...
        # Make request
        response = client.patch(
            f"/api/v1/water-heaters/{sample_water_heater.id}/mode", json={"mode": "ECO"}
        )
    ...
    @patch("src.api.water_heater.configurable_router.ConfigurableWaterHeaterService")
    def test_get_health_configuration(
        self, mock_service_class, mock_service, sample_health_config
    ):
        """Test getting health configuration."""
        # Setup mock
        mock_service_class.return_value = mock_service
        mock_service.repository.get_health_configuration = AsyncMock(
            return_value=sample_health_config
        )
    ...
    @patch("src.api.water_heater.configurable_router.ConfigurableWaterHeaterService")
    def test_set_health_configuration(
        self, mock_service_class, mock_service, sample_health_config
    ):
        ...
        # Make request
        response = client.post(
            "/api/v1/water-heaters/health-configuration", json=sample_health_config
        )
        ...
        # Verify service was called
        mock_service.repository.set_health_configuration.assert_called_once_with(
    ...
    def test_get_alert_rules(self, mock_service_class, mock_service, sample_alert_rule):
        ...
        # Setup mock
        mock_service_class.return_value = mock_service
        mock_service.repository.get_alert_rules = AsyncMock(
            return_value=[sample_alert_rule]
        )
    ...
    def test_add_alert_rule(self, mock_service_class, mock_service, sample_alert_rule):
        ...
        # Setup mock
        mock_service_class.return_value = mock_service
        mock_service.repository.add_alert_rule = AsyncMock(
            return_value=sample_alert_rule
        )

        # Make request
        response = client.post(
            "/api/v1/water-heaters/alert-rules", json=sample_alert_rule
        )
        ...
        # Verify service was called
        mock_service.repository.add_alert_rule.assert_called_once_with(
            sample_alert_rule
        )

    @patch("src.api.water_heater.configurable_router.ConfigurableWaterHeaterService")
    def test_update_alert_rule(
        self, mock_service_class, mock_service, sample_alert_rule
    ):
        ...
        # Verify service was called
        mock_service.repository.update_alert_rule.assert_called_once_with(
            sample_alert_rule["id"], updated_rule
        )
    ...
    def test_delete_alert_rule(
        ...
        # Verify service was called
        mock_service.repository.delete_alert_rule.assert_called_once_with(
