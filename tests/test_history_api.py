    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params) as response:
            ...
            if status == 200:
                ...
                logger.info(
                    f"✅ Temperature history endpoint returned status 200 with data"
                )
                if "datasets" in data and len(data["datasets"]) > 0:
                    logger.info(
                        f"   - Contains {len(data['datasets'][0]['data'])} data points"
                    )
            else:
                text = await response.text()
                logger.error(
                    f"❌ Temperature history endpoint failed with status {status}: {text}"
                )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params) as response:
            ...
            if status == 200:
                data = await response.json()
                logger.info(
                    f"✅ Energy usage history endpoint returned status 200 with data"
                )
                if "datasets" in data and len(data["datasets"]) > 0:
                    logger.info(
                        f"   - Contains {len(data['datasets'][0]['data'])} data points"
                ...
                text = await response.text()
                logger.error(
                    f"❌ Energy usage history endpoint failed with status {status}: {text}"
                )
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params) as response:
            ...
            if status == 200:
                data = await response.json()
                logger.info(
                    f"✅ Pressure flow history endpoint returned status 200 with data"
                )
                if "datasets" in data and len(data["datasets"]) > 0:
                    logger.info(
                        f"   - Contains {len(data['datasets'][0]['data'])} data points"
                ...
                text = await response.text()
                logger.error(
                    f"❌ Pressure flow history endpoint failed with status {status}: {text}"
                )
...
async def test_history_dashboard():
    ...
    url = (
        f"{BASE_URL}/api/manufacturer/water-heaters/{TEST_HEATER_ID}/history/dashboard"
    )
    params = {"days": 7}
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url, params=params) as response:
            ...
            if status == 200:
                data = await response.json()
                logger.info(
                    f"✅ History dashboard endpoint returned status 200 with data"
                )
                keys = data.keys()
                ...
                text = await response.text()
                logger.error(
                    f"❌ History dashboard endpoint failed with status {status}: {text}"
                )
