...
class TestWebSocketService(unittest.TestCase):
    ...
    def setUp(self):
        ...
        from src.infrastructure.websocket.websocket_service import (
            WebSocketClient,
            WebSocketService,
        )
    ...
    def test_handle_device_telemetry(self):
        ...
        event = {
            ...
            "data": {"temperature_current": 120.5, "heating_status": True},
            "simulated": True,
        }
    ...
    def test_start_service(self, mock_get_event_loop):
        ...
        self.message_bus.subscribe.assert_any_call(
            "device.telemetry", self.websocket_service.handle_device_telemetry
        )
        self.message_bus.subscribe.assert_any_call(
            "device.event", self.websocket_service.handle_device_event
        )
        self.message_bus.subscribe.assert_any_call(
            "device.command_response", self.websocket_service.handle_command_response
        )
    ...
    def test_stop_service(self):
        ...
        self.message_bus.unsubscribe.assert_any_call(
            "device.telemetry", self.websocket_service.handle_device_telemetry
        )
        self.message_bus.unsubscribe.assert_any_call(
            "device.event", self.websocket_service.handle_device_event
