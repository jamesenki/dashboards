...
class TestWebSocketEndpoints(unittest.TestCase):
    ...
    async def test_websocket_connect(self):
        ...
        with patch("src.services.websocket_manager.WebSocketManager") as mock_manager:
            ...
            with patch(
                ...
                return_value=mock_manager.return_value,
            ):
                with patch("fastapi.WebSocket") as mock_websocket:
                    ...
                    with patch("src.api.routes.websocket.device_state") as mock_handler:
                        ...
                        mock_manager.return_value.connect.assert_called_once_with(
                            mock_websocket, device_id
                        )
                        mock_manager.return_value.disconnect.assert_called_once_with(
                            mock_websocket, device_id
                        )
    ...
    async def test_device_state_updates(self):
        ...
        state_update = {
            ...
            "state": {"temperature": 66.5, "pressure": 2.4, "mode": "eco"},
            "version": 4,
        }
        ...
        with patch("src.services.websocket_manager.WebSocketManager") as mock_manager:
            ...
            with patch(
                "src.services.device_shadow.DeviceShadowService"
            ) as mock_shadow_service:
                ...
                with patch("fastapi.WebSocket") as mock_websocket:
                    ...
                    with patch("src.api.routes.websocket.device_state") as mock_handler:
                            ...
                            try:
                                while True:
                                    ...
                                    if data == "close":
                                        ...
                                        state = await shadow_service.get_device_shadow(
                                            device_id
                                        )
                        ...
                        # Verify the behavior
                        mock_websocket.accept.assert_called_once()
                        shadow_instance.get_device_shadow.assert_called_once_with(
                            device_id
                        )
    ...
    async def test_device_telemetry_stream(self):
        ...
        with patch("src.services.websocket_manager.WebSocketManager") as mock_manager:
            ...
            with patch(
                "src.services.telemetry_service.TelemetryService"
            ) as mock_telemetry:
                ...
                telemetry_instance.subscribe_to_device_telemetry.side_effect = (
                    simulate_telemetry
                )
                ...
                with patch("fastapi.WebSocket") as mock_websocket:
                    ...
                    mock_websocket.receive_text = AsyncMock(
                        side_effect=["subscribe", asyncio.TimeoutError, "close"]
                    )
                    mock_websocket.send_text = AsyncMock()
                    mock_websocket.close = AsyncMock()
                    ...
                    with patch(
                        "src.api.routes.websocket.device_telemetry"
                    ) as mock_handler:
                        # Set up the mock to call our test code
    ...
    async def test_websocket_authentication(self):
        ...
        with patch("src.services.auth_service.AuthService") as mock_auth:
            ...
            with patch("fastapi.WebSocket") as mock_websocket:
                ...
                with patch(
                    "src.api.routes.websocket.authenticate_connection"
