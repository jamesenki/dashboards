...
class TestShadowDocumentE2E:
    ...
    def test_shadow_document_e2e(self):
        ...
        response = requests.put(
            f"{BASE_URL}/api/device/{TEST_DEVICE_ID}/shadow", json=shadow_data
        )
        ...
        assert (
            shadow["state"]["reported"]["temperature"] == 60.5
        ), "Temperature should be updated"
        assert (
            shadow["state"]["reported"]["status"] == "online"
        ...
        new_shadow_data = {"state": {"reported": {"temperature": 62.0}}}

        requests.put(
            f"{BASE_URL}/api/device/{TEST_DEVICE_ID}/shadow", json=new_shadow_data
        )
        ...
        assert (
            update["data"]["state"]["reported"]["temperature"] == 62.0
        ), "WebSocket should receive temperature update"
