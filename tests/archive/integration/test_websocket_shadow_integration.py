...
async def test_device_shadow_websocket_integration():
    ...
    try:
        async with websockets.connect(ws_url) as websocket:
            ...
            logger.info(
                f"Received initial state: {json.dumps(initial_state, indent=2)}"
            )
            ...
            logger.info(
                f"Received current state: {json.dumps(current_state, indent=2)}"
            )

            # Update the desired state
            new_desired_state = {"target_temperature": 68.0, "mode": "eco"}

            logger.info(
                f"Updating desired state: {json.dumps(new_desired_state, indent=2)}"
            )
            ...
            logger.info(
                f"Received update confirmation: {json.dumps(update_result, indent=2)}"
            )

            # Request delta between reported and desired
            await websocket.send(json.dumps({"type": "get_delta"}))
...
async def test_device_telemetry_websocket():
    ...
    try:
        async with websockets.connect(ws_url) as websocket:
            ...
            await websocket.send(
                json.dumps(
                    {"type": "subscribe", "metrics": ["temperature", "pressure"]}
                )
            )
            ...
            logger.info(
                f"Received subscription confirmation: {json.dumps(subscription, indent=2)}"
            )
            ...
            try:
                ...
                for _ in range(5):
                    ...
                    try:
                        telemetry_response = await asyncio.wait_for(
                        ...
                        logger.info(
                            f"Received telemetry: {json.dumps(telemetry, indent=2)}"
                        )
                ...
                logger.info(
                    f"Unsubscribed from telemetry. Received {telemetry_received} updates."
                )
...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        logger.info(
            f"Device Telemetry WebSocket: {'PASS' if telemetry_result else 'FAIL'}"
        )
