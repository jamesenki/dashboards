...
def create_test_token(user_id: str, username: str, role: str = UserRole.FACILITY_MANAGER, expired: bool = False):
    """Create a test JWT token with configurable expiration"""
    expiration = datetime.utcnow() + (timedelta(minutes=-1) if expired else timedelta(days=1))
...
async def test_websocket_connection_role_based_access():
    ...
    readonly_token = create_test_token(TEST_USER_ID, TEST_USERNAME, role=UserRole.READ_ONLY)
