...
class TestSQLiteWaterHeaterRepository:
    ...
    @pytest.mark.asyncio
    async def test_add_multiple_readings(
        self, repository, sample_water_heater, sample_reading
    ):
        ...
        limited_readings = await repository.get_readings(
            sample_water_heater.id, limit=3
        )
    ...
    async def test_nonexistent_water_heater(self, repository):
        ...
        result = await repository.update_water_heater(
        ...
        reading = WaterHeaterReading(
            id=str(uuid.uuid4()), temperature=50.0, timestamp=datetime.now()
        )
