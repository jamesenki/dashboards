...
def create_test_alert():
    ...
    try:
        ...
        if not rule:
            ...
            active_column = 'active' if 'active' in column_names else 'is_active' if 'is_active' in column_names else None
            operator_column = 'operator' if 'operator' in column_names else 'condition' if 'condition' in column_names else None
            ...
            cursor.execute(f"INSERT INTO alert_rules ({cols}) VALUES ({placeholders})", values)
            conn.commit()
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        """, (event_id, rule_id, model_id, metric_name, metric_value, severity, timestamp, 0))
