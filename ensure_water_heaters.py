#!/usr/bin/env python3
"""
Script to ensure all 8 water heaters are properly registered in the AssetDB
and have shadow documents with complete historical data.

This script:
1. Registers 8 water heaters in the asset registry (if not already present)
2. Creates shadow documents for each water heater (if not already present)
3. Adds 7 days of historical data to each shadow document
4. Creates a backup data file that the frontend can use if API calls fail
"""
import asyncio
import json
import logging
...
# 8 water heaters with consistent IDs
WATER_HEATER_IDS = [
...
async def ensure_water_heaters():
    """Create or update asset registry and shadow documents for all water heaters."""
    from src.services.asset_registry import AssetRegistryService
    from src.services.device_shadow import DeviceShadowService

    logger.info(
        f"Creating asset registry entries and shadow documents for {len(WATER_HEATER_IDS)} water heaters"
    )
    shadow_service = DeviceShadowService()
    registry_service = AssetRegistryService()

    # Track all water heaters for backup data
    all_water_heaters = []
    ...
    for heater_id in WATER_HEATER_IDS:
        ...
        try:
            ...
            device_data = {
                "device_id": heater_id,  # Use device_id instead of id for AssetRegistryService
            ...
            try:
                ...
                device = device_data  # Use our data even if registration failed

        # Add to our tracking list for backup data
        if isinstance(device, dict):
            all_water_heaters.append(device)
        ...
        if shadow_exists:
            ...
            readings = await generate_shadow_history(shadow_service, heater_id, days=7)

            # Update the all_water_heaters entry with more details
            for heater in all_water_heaters:
                if (
                    heater.get("device_id") == heater_id
                    or heater.get("id") == heater_id
                ):
                    try:
                        shadow = await shadow_service.get_device_shadow(heater_id)
                        if isinstance(shadow, dict) and "reported" in shadow:
                            for key, value in shadow["reported"].items():
                                heater[key] = value
                        heater["readings"] = readings
                    except Exception as e:
                        logger.warning(
                            f"Could not get updated shadow data for {heater_id}: {e}"
                        )

    # 4. CREATE BACKUP DATA FILE
    create_backup_data_file(all_water_heaters)

    logger.info(
        f"Successfully ensured {len(all_water_heaters)} water heaters exist with complete shadow data"
    )
...
async def generate_shadow_history(shadow_service, heater_id, days=7):
    ...
    try:
        ...
        if isinstance(shadow, dict):
            # Add readings array to the main shadow object
            ...
            # Also add to the reported section (where the frontend often looks)
...
def create_backup_data_file(water_heaters):
    """
    Create a backup JSON file with water heater data for the frontend to use.

    Args:
        water_heaters: List of water heater objects with their data
    """
    # Format water heaters for frontend use
    formatted_heaters = []

    for heater in water_heaters:
        # Create a standardized version of each water heater
        heater_id = heater.get("device_id") or heater.get("id")
        formatted_heater = {
            "id": heater_id,
            "name": heater.get("name", f"Water Heater {heater_id.split('-')[1]}"),
            "manufacturer": heater.get("manufacturer", "AquaSmart"),
            "model": heater.get("model", "SmartTank Pro"),
            "type": "water_heater",
            "status": heater.get("status", "ONLINE"),
            "temperature": heater.get("temperature", 120.0),
            "target_temperature": heater.get("target_temperature", 125.0),
            "pressure": heater.get("pressure", 60.0),
            "flow_rate": heater.get("flow_rate", 3.2),
            "energy_usage": heater.get("energy_usage", 450.0),
            "mode": heater.get("mode", "ECO"),
            "heater_status": heater.get("heater_status", "STANDBY"),
            "readings": heater.get("readings", []),
        }
        formatted_heaters.append(formatted_heater)

    # Ensure we have all 8 water heaters in the backup file
    existing_ids = set(h["id"] for h in formatted_heaters)
    for heater_id in WATER_HEATER_IDS:
        if heater_id not in existing_ids:
            # Create a default water heater if missing
            formatted_heaters.append(
                {
                    "id": heater_id,
                    "name": f"Water Heater {heater_id.split('-')[1]}",
                    "manufacturer": "AquaSmart",
    ...
    # Create the backup file in the frontend static directory
    ...
    with open(backup_file, "w") as f:
        json.dump(formatted_heaters, f, indent=2)

    logger.info(
        f"Created backup data file with {len(formatted_heaters)} water heaters at {backup_file}"
    )


if __name__ == "__main__":
    asyncio.run(ensure_water_heaters())
