    ...
    try:
        ...
        headers = [
            (
                "User-Agent",
                "TestClient/1.0",
            ),  # Using TestClient to trigger the test bypass in middleware
            (
                "Authorization",
                f"Bearer {token}",
            ),  # Also include token in header as fallback
        ]
        ...
        try:
            ...
            logger.error(
                f"WebSocket connection failed with status {status_code} after {duration:.2f}s"
            )
            ...
            duration = time.time() - start_time
            logger.error(
                f"WebSocket connection closed unexpectedly after {duration:.2f}s: {e}"
            )
