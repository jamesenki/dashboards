...
async def populate_shadow_documents(devices: List[str], exclude: List[str] = None):
    ...
    for device_id in devices:
        ...
        if exclude and device_id in exclude:
            logger.info(f"Skipping shadow creation for {device_id} (excluded for testing)")
            continue
        ...
        try:
            ...
            await generate_telemetry_history(device_id, telemetry_service, base_temp, days=7)
            
        except Exception as e:
            logger.error(f"Error creating shadow for {device_id}: {str(e)}")

async def generate_telemetry_history(device_id: str, telemetry_service: TelemetryService, 
                                    base_temp: float, days: int = 7):
    """
    ...
    logger.info(f"Successfully generated {len(data_points)} telemetry points for {device_id}")

def main():
    """Main entry point for the script"""
    parser = argparse.ArgumentParser(description="Populate shadow documents for testing")
    parser.add_argument(
        "--devices", 
        nargs="+", 
