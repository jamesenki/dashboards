...
def main():
    ...
    if devices_data:
        logger.info(
            f"Found {len(devices_data.get('devices', []))} water heater devices in mock data"
        )
    ...
    if metrics_data:
        ...
        logger.info(
            f"  - Active devices: {metrics_data.get('metrics', {}).get('active_devices')}"
        )
        logger.info(
            f"  - Alerts triggered: {metrics_data.get('metrics', {}).get('alerts_triggered')}"
        )
