        ...
        if models_response.status_code == 200:
            ...
            if model_count > 0:
                ...
                if isinstance(test_model, dict):
                    ...
                    if (
                        "versions" in test_model
                        and isinstance(test_model["versions"], list)
                        and test_model["versions"]
                    ):
                ...
                logger.info(
                    f"Testing with model_id: {model_id}, version: {model_version}"
                )
                ...
                logger.info(
                    f"Alert rules endpoint returned status: {rules_response.status_code}"
                )
                ...
                logger.info(
                    f"Alerts endpoint returned status: {alerts_response.status_code}"
                )
                ...
                if alerts_response.status_code == 200:
                    ...
                    logger.info(
                        f"Alerts count: {len(alerts)}, is_mock: {is_mock_alerts}"
                    )
