...
class AlertWorkflowTest:
        ...
        try:
            ...
            if model:
                ...
                cursor.execute(
                    ...
                    (
                        TEST_MODEL_ID,
                        "Water Heater Test Model",
                        "Test model for alerts",
    ...
    def test_step2_create_alert_rule(self):
        ...
        try:
            ...
            if rule:
                ...
                logger.info(
                    f"Rule details: {json.dumps({k: rule[k] for k in rule.keys()}, default=str)}"
                )
                return True
            else:
    ...
    def test_step3_create_alert_event(self):
        ...
        try:
            ...
            if alert:
                ...
                logger.info(
                    f"Alert details: {json.dumps({k: alert[k] for k in alert.keys()}, default=str)}"
                )
                return True
            else:
        ...
        try:
            ...
            for alert in alerts:
                ...
                if alert_id == self.alert_id:
                    ...
                    logger.info(
                        f"Found our test alert in API response: {json.dumps(alert)}"
                    )
                    break
            ...
            if not test_alert_found:
                ...
                model_alerts_url = (
                    f"{API_BASE_URL}/models/{TEST_MODEL_ID}/versions/1.0/alerts"
                )
                ...
                if model_response.status_code != 200:
                    logger.error(
                        f"Model API request failed with status code: {model_response.status_code}"
                    )
                else:
    ...
    def test_step5_check_frontend_alerts(self):
        ...
        try:
            ...
            if response.status_code != 200:
                logger.error(
                    f"Frontend request failed with status code: {response.status_code}"
                )
                return False
            ...
            if self.alert_id in frontend_html:
                logger.info(
                    f"Found our test alert ID in the frontend HTML: {self.alert_id}"
                )
                return True
            else:
                logger.warning(
                    f"Our test alert ID was not found in the frontend HTML: {self.alert_id}"
                )
                logger.info(
                    "This doesn't necessarily mean it won't appear - the frontend loads alerts via JavaScript"
                )
                ...
                logger.info(
                    "2. Check if the Recent Alerts section contains your test alert"
                )
        ...
        try:
            ...
            if fetch_url_line:
                ...
                if "/api/monitoring/alerts" not in fetch_url_line:
                    logger.warning(
                        f"Frontend is not fetching from /api/monitoring/alerts"
                    )
            ...
            if '@app.get("/alerts")' in dashboard_api:
                ...
                logger.warning(
                    "Could not find /alerts endpoint definition in dashboard_api.py"
                )
