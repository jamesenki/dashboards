...
class VendingMachineOperationsService:
    ...
    def get_sales_data(
        self, machine_id: str, period: Union[str, SalesPeriod] = SalesPeriod.WEEK
    ) -> SalesData:
        ...
        sales_data = self._generate_mock_sales_data(
            machine, period, start_date, end_date
        )
        ...
        if readings:
            temp_readings = [
                TemperatureReading(
                    ...
                    is_normal=True
                    if reading.temperature and -10 <= reading.temperature <= 10
                    else False,
                )
    ...
    def _generate_mock_sales_data(
        ...
        for product_sale in product_sales:
            if total_revenue > 0:
                product_sale.percentage_of_total = (
                    product_sale.revenue / total_revenue
                ) * 100
            else:
    ...
    def _generate_mock_maintenance_history(
        ...
        for i in range(num_events):
            ...
            event = MaintenanceEvent(
                ...
                if maint_type == MaintenanceType.REPAIR
                else None,
        ...
        for i in range(num_events):
            ...
            for product_id, name, _ in refilled_products:
                ...
                refill_item = RefillItem(
                    product_id=product_id, name=name, quantity=quantity, cost=cost
                )
                ...
                if product_id in product_refill_counts:
                    product_refill_counts[product_id] = (
                        product_refill_counts[product_id][0] + quantity,
                        name,
                    )
                else:
    ...
    def _generate_mock_temperature_trends(
        ...
        for hours in range(0, 24 * 7, 2):
            ...
            reading = TemperatureReading(
                timestamp=timestamp, temperature=temperature, is_normal=is_normal
            )
