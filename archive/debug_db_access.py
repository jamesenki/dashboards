...
async def main():
    ...
    try:
        ...
        for model in sql_models:
            logger.info(
                f"SQL Model: {model['id']} (data_source: {model.get('data_source', 'Not set')})"
            )
    ...
    try:
        ...
        logger.info(
            f"Got {len(models)} models from metrics repository (is_mock={is_mock})"
        )
        for model in models:
            logger.info(
                f"Model: {model['id']} (data_source: {model.get('data_source', 'Not set')})"
            )
    except Exception as e:
    ...
    try:
        ...
        logger.info(
            f"Got {len(service_models)} models from service (is_mock={is_mock})"
        )
        for model in service_models:
            logger.info(
                f"Service Model: {model['id']} (data_source: {model.get('data_source', 'Not set')})"
            )
