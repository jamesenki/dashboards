...
async def get_real_alerts_from_db():
    ...
    try:
        ...
        for alert in db_alerts:
            ...
            op_text = "exceeded" if alert.get('condition', '>') in ['>', '>='] else "dropped below"
            formatted_alert["description"] = f"{formatted_alert['metric_name'].replace('_', ' ').title()} {op_text} threshold"
...
async def test_alerts_api():
    ...
    logger.info(f"Current API returns {len(current_alerts)} alerts: {json.dumps(current_alerts[:1], indent=2)}")
    ...
    if real_alerts:
        logger.info(f"Sample alert from database: {json.dumps(real_alerts[0], indent=2)}")
    ...
    logger.info("Visit http://localhost:8006/api/monitoring/real_alerts to see real alerts")
    logger.info("\nInstructions to fix the issue:")
    logger.info("1. Modify the frontend code in alerts.html to temporarily use /api/monitoring/real_alerts")
    logger.info("2. Debug why the main /api/monitoring/alerts endpoint isn't returning real data")
    logger.info("3. Update dashboard_api.py to ensure it's retrieving data from the database correctly")
...
def patch_frontend():
    ...
    try:
        ...
        modified_content = content.replace(
            ...
            "fetch(`/api/monitoring/real_alerts`)"
        )
