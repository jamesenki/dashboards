...
def inspect_alert_events_directly():
    ...
    try:
        ...
        if count > 0:
            ...
            for alert in alerts:
                logger.info(
                    json.dumps({k: alert[k] for k in alert.keys()}, default=str)
                )
...
def create_test_alert():
    ...
    try:
        ...
        cursor.execute(
            "SELECT id, model_id, metric_name, threshold, condition FROM alert_rules LIMIT 1"
        )
        ...
        if not rule:
            ...
            cursor.execute(
                "SELECT id, model_id, metric_name, threshold, condition FROM alert_rules WHERE id = ?",
                (rule_id,),
            )
            rule = cursor.fetchone()
...
def format_alerts_as_frontend_expects():
    ...
    try:
        ...
        logger.info(
            f"Wrote {len(formatted_alerts)} formatted alerts to expected_alerts_format.json"
        )
...
async def check_dashboard_api_function():
    ...
    logger.info(
        "1. Check that the '/alerts' endpoint definition uses the correct database query"
    )
    logger.info(
        "2. Ensure alerts are formatted correctly to match frontend expectations"
    )
    logger.info(
        "3. Verify error handling returns an empty array instead of raising exceptions"
    )
    logger.info(
        "4. Confirm response fields match frontend expectations (id, rule_name, metric_name, etc.)"
    )
