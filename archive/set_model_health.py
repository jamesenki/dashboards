...
def main():
    ...
    try:
        ...
        health_weights = [
            ...
            0.3,
            0.1,
        ]  # Weights for random selection (60% healthy, 30% drifting, 10% degraded)
        ...
        for model_version in model_versions:
            ...
            metrics_summary = {
                "accuracy": round(
                    random.uniform(0.85 if health_status == "healthy" else 0.70, 0.98),
                    4,
                ),
                "precision": round(
                    random.uniform(0.85 if health_status == "healthy" else 0.70, 0.98),
            ...
            drift_metrics = {
                "drift_score": round(
                    random.uniform(
                        0.02 if drift_status == "normal" else 0.15,
                        0.08 if drift_status == "normal" else 0.35,
                    ),
                    4,
                ),
                "data_quality_score": round(
                    random.uniform(0.85 if drift_status == "normal" else 0.60, 0.95), 4
                ),
            }
            ...
            if existing_record:
                logger.info(
                    f"Updating health status for {model_name} ({model_id}) version {version} to {health_status}"
                )
                cursor.execute(
                    """
                UPDATE model_health
                ...
                logger.info(
                    f"Creating health status for {model_name} ({model_id}) version {version} as {health_status}"
                )
                cursor.execute(
                    """
        ...
        for model in all_models:
            ...
            if version_row:
                ...
                if health_row:
                    ...
                    logger.info(
                        f"Updated health reference for model {model_id} to {health_status}"
                    )
