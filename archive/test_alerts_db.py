...
async def main():
    ...
    try:
        ...
        logger.info(
            f"Alert rules table columns: {[col['name'] for col in rule_columns]}"
        )
        ...
        logger.info(
            f"Alert events table columns: {[col['name'] for col in event_columns]}"
        )
    except Exception as e:
    ...
    try:
        ...
        logger.info(
            f"SQLiteModelMetricsRepository.get_alert_rules returned: {type(sql_rules)}"
        )
        ...
        logger.info(
            f"SQLiteModelMetricsRepository.get_triggered_alerts returned: {type(sql_alerts)}"
        )
    ...
    try:
        ...
        logger.info(
            f"ModelMetricsRepository.get_alert_rules returned is_mock={is_mock_rules}"
        )
        ...
        # Get alerts
        repo_alerts, is_mock_alerts = await metrics_repo.get_triggered_alerts(
            test_model_id, test_version
        )
        logger.info(
            f"ModelMetricsRepository.get_triggered_alerts returned is_mock={is_mock_alerts}"
        )
    ...
    try:
        ...
        logger.info(
            f"ModelMonitoringService.get_alert_rules returned is_mock={is_mock_rules}"
        )
        ...
        # Get alerts
        service_alerts, is_mock_alerts = await service.get_triggered_alerts(
            test_model_id, test_version
        )
        logger.info(
            f"ModelMonitoringService.get_triggered_alerts returned is_mock={is_mock_alerts}"
        )
