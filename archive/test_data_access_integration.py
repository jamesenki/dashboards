...
def run_tests():
    ...
    try:
        ...
        db = initialize_database(
            db_path=temp_db_path, in_memory=False, populate=True, force_reset=True
        )

        # Set up configuration for tests
        logger.info("Setting up test configuration")
        config_service = ConfigurationService()
        test_config = {"testing": {"mocks": {"enabled": True, "data_path": "./mocks"}}}
        ...
        with patch("src.api.data_access.config", config_service):
            ...
            logger.info(
                f"DataAccessLayer initialized, using mocks: {data_access.is_using_mocks()}"
            )

            # Create monitoring service with the data access layer
            with patch(
                "src.monitoring.model_monitoring_service_sync.data_access", data_access
            ):
...
def test_get_model_details(service):
    ...
    logger.info(
        f"Successfully retrieved model details: {water_heater_model.get('name')}"
    )

    # Test with a non-existent model
    nonexistent_model = service.get_model_details("nonexistent-model")
    assert (
        nonexistent_model is not None
    ), "Should get fallback model details for nonexistent model"
    assert (
        nonexistent_model["id"] == "nonexistent-model"
    ), "Model ID should match request"
...
def test_get_model_metrics(service):
    ...
    logger.info(
        f"Successfully retrieved metrics with {len(metrics.get('metrics_history', []))} records"
    )
...
def test_record_metrics(service):
    ...
    model_version = "1.0.0"
    metrics = {"accuracy": 0.92, "precision": 0.90, "recall": 0.89}
