...
for model_id, model_version in [
    (m["model_id"], m["model_version"]) for m in metric_entries
]:
    ...
    for metric_name in metrics_to_check:
        ...
        if not has_metric:
            print(
                f"Adding {metric_name} metric for {model_id} (version {model_version})"
            )
            ...
            cursor.execute(
                ...
                (
                    f"{model_id}-{model_version}-{metric_name}-{datetime.now().timestamp()}",
                    model_id,
                    model_version,
                    metric_name,
                    metric_value,
