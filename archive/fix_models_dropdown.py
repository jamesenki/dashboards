...
def verify_model_exists():
    ...
    try:
        ...
        if model:
            ...
            logger.warning(f"❌ Energy Consumption forecaster model not found in database")
            return False
...
def verify_alerts():
    ...
    try:
        ...
        if count > 0:
            logger.info(f"✅ Found {count} alerts for Energy Consumption forecaster model")
...
def main():
    ...
    if not verify_model_exists():
        ...
        if not create_energy_model():
            logger.error("Failed to create Energy Consumption forecaster model. Aborting.")
            return
    ...
    if verify_alerts():
        logger.info("✅ Fix completed successfully! Energy Consumption forecaster should now appear in Alerts Tab.")
        logger.info("Please refresh the Alerts Tab in your browser to see the changes.")
    else:
        logger.error("❌ Fix failed. Energy Consumption forecaster might not appear in Alerts Tab.")
