...
def check_python_code(path: str, fix: bool = False) -> bool:
    ...
    isort_args = ["isort", "--check-only", "--profile", "black", path] if not fix else ["isort", "--profile", "black", path]
...
def check_javascript_code(path: str, fix: bool = False) -> bool:
    ...
    eslint_args = ["npx", "eslint", path] if not fix else ["npx", "eslint", "--fix", path]
    ...
    prettier_args = ["npx", "prettier", "--check", path] if not fix else ["npx", "prettier", "--write", path]
...
def check_html_css(path: str, fix: bool = False) -> bool:
    ...
    stylelint_args = ["npx", "stylelint", f"{path}/**/*.css"] if not fix else ["npx", "stylelint", f"{path}/**/*.css", "--fix"]
...
def check_docstrings(path: str) -> bool:
    ...
    exit_code, output = run_command(["pydocstyle", "--convention=google", path], cwd=project_root)
    if exit_code != 0:
...
def main() -> int:
    ...
    parser.add_argument(
        "--python", 
        action="store_true", 
        help="Only check Python code"
    )
    parser.add_argument(
        "--javascript", 
        action="store_true", 
        help="Only check JavaScript code"
    )
    parser.add_argument(
        "--html-css", 
        action="store_true", 
        help="Only check HTML/CSS"
    )
    parser.add_argument(
        "--security", 
        action="store_true", 
    ...
    run_all = not (args.python or args.javascript or args.html_css or args.security or args.docstrings)
    success = True
