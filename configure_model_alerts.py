...
def main():
    ...
    try:
        ...
        logger.info(f"Found {len(model_versions)} model versions to configure alerts for")
        ...
        for model_version in model_versions:
            ...
            logger.info(f"Configuring alerts for {model_name} ({model_id}) version {version} using {template_key if template_key else 'default'} template")
            ...
            logger.info(f"Added {len(alert_rules)} alert rules for {model_name} ({model_id}) version {version}")
            ...
            if random.random() < 0.6:  # 60% chance of having alert events
                ...
                if rules:
                    ...
                    for rule in selected_rules:
                        ...
                        if metric_name.startswith("accuracy") or metric_name.startswith("precision") or metric_name.startswith("recall") or metric_name.startswith("f1"):
                        ...
                        resolved_at = (now - timedelta(days=random.randint(0, 5))).isoformat() if resolved else None
                    ...
                    logger.info(f"Added {len(events)} alert events for {model_name} ({model_id}) version {version}")
