...
class TestWaterHeaterViewConsistency:
    ...
    def test_water_heater_details_consistency(self):
        ...
        water_heater_detail_response = client.get(
            f"/api/water-heaters/{self.heater_id}"
        )
        ...
        assert (
            page_heater_id == self.heater_id
        ), f"Expected URL to contain heater ID {self.heater_id}, got {page_heater_id}"
    ...
    def test_operations_tab_consistency(self):
        ...
        operations_response = client.get(
            f"/api/water-heaters/{self.heater_id}/operations"
        )
        ...
        assert operations_response.status_code in (
            200,
            404,
        ), f"Unexpected status code: {operations_response.status_code}"
        ...
        if operations_response.status_code == 200:
            ...
            if (
                "current_temperature" in operations_data
                and "current_temp" in details_data
            ):
                # Allow small difference due to potential updates between calls
                assert (
                    abs(
                        operations_data["current_temperature"]
                        - details_data["current_temp"]
                    )
                    < 5
            ...
            assert soup.select_one(
                "#operations-content"
            ), "Operations tab content not found"
    ...
    def test_history_tab_consistency(self):
        ...
        assert (
            "energy_usage" in history_data
        ), "History data missing energy_usage section"
        assert (
            "pressure_flow" in history_data
        ), "History data missing pressure_flow section"

        # Get temperature history specifically to check ID
        temp_history_response = client.get(
            f"/api/water-heaters/{self.heater_id}/history/temperature"
        )
        assert temp_history_response.status_code == 200

        # Get energy usage history to check ID
        energy_history_response = client.get(
            f"/api/water-heaters/{self.heater_id}/history/energy"
        )
        assert energy_history_response.status_code == 200

        # Get pressure-flow history to check ID
        pressure_flow_history_response = client.get(
            f"/api/water-heaters/{self.heater_id}/history/pressure-flow"
        )
    ...
    def test_end_to_end_consistency(self):
        ...
        assert soup.select_one(
            "#details-content"
        ), "Details content container not found"
        assert soup.select_one(
            "#operations-content"
        ), "Operations content container not found"
        assert soup.select_one(
            "#history-content"
        ), "History content container not found"
